...
- **まだ引かれていない古い発言**: 照合は引用を含む最新の著者の発言に結び、 既に引かれた発言を著者の次の発言の後に別の案へ使い回すことは止める (exit 5)。 だが一度も引かれていない古い発言は後から引ける。 記録が遅れた正当な場合と、 著者が見ていない案への流用を、 機械は区別しない。 記録と approve の出力には発言の時刻と冒頭が残るので、 著者が後から読める。
