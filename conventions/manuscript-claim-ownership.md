...
- **macro で包んだ数式環境も保護領域**: 原稿が display 数式を 1 引数の macro で包んでいる場合 (`\newcommand{\al}[1]{\begin{align}#1\end{align}}` 型。 `\def\al#1{…}`・xparse の `{m}`・本体が `\[#1\]` のものも)、 その macro の brace の引数を上の数式と同じ識別で読む。 定義は、 編集する file を含む原稿 (abstract を持つ `.tex` か設定の `include` の `.tex` から、 `\input` 系・`\usepackage` 系・`\documentclass` 系で辿れる `.tex` / `.sty` / `.cls` / `.def` / `.clo`) から拾う。 読み込み先は参照元の dir と repo の root で探し、 `\usepackage` 系・`\documentclass` 系で見つからなければ同じ名前の file を全部読む (`\input` 系の解決できない行は辿らない)。 版は作業ツリー (ignore されていない未追跡を含む)・index・HEAD のすべてで、 原稿の起点になるのは追跡済みの file と編集中の file だけ。 読み込みと定義を探すときは verbatim 系の中を読まない。 拾えない定義 (repo の外の preamble、 別の書き方) は `.claude/manuscript-guard.json` の `math_macros` に macro 名を列挙する。 その原稿の範囲で wrapper の名前の定義が変わる変更 (消す・wrapper と読めない形に書き換える・再定義を足す・定義の file の読み込みを外す) も、 式の変更として止める。 原稿の範囲の外の file (`.sty` など) の編集でも、 原稿が読み込む file ならこの検査だけは受ける。 包む環境の種類 (`align` と `align*` など) の差は、 直の環境と同じく変更とみなさない。 定義の変更を著者が承認した後は、 新しい定義で読む (wrapper でなくなった macro の引数は式として読まない)。 引数が 2 つ以上の wrapper と、 brace を使わない引数は見ない。
...
- **引用の意味**: 「直して」 を概要の削除の裁定として記録することは機械では止まらない。 代わりに、 記録したターンの最後の返事に引いた発言と対象の file を書くことを Stop hook が求める (無ければ 1 回差し戻す) = 著者はその場で見て止められる。 §1-3 が床。
