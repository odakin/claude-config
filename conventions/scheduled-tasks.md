...
| **launchd cron + `claude -p`** (既定) | transcript は local `~/.claude/projects/<dir>/<session-id>.jsonl` に保存される。 ⚠️ さらに **settings で `remoteControlAtStartup: true` のマシンでは headless run が Remote Control session として claude.ai に登録され「最近の項目」 に出ることがある** (= `--remote-control-session-name-prefix` default = hostname による自動命名 `<hostname>-<codename>`。 2026-07-04 実測: 30 分 cron の完走 run が別マシンの一覧に出現 + run session が終了せず**数十件 live で積み上がる**事例)。 ⚠️ ただし**発生条件は未確定**: 同日 A/B で、 同設定 (remoteControlAtStartup: true) の別マシン CLI 2.1.198 の plain `-p` 単発は recents に**出なかった** (= version / 環境 config 依存の可能性、 どちらが効いているか切り分け未了 → だからこそ下記の両 flag を既定にする)。 ⚠️ errata (同日): 初版の「recents に出ない実測」 は standby マシン (= active-routine-host gate defer で cron が実行されない側) での観測で、 **観測として無効**だった |
