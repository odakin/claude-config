...
7. **描いた印・図形・画像の検収は、 数値の検査 (面積比・個数・有無) が通っても、 紙の大きさと高い倍率の両方の crop で見る**。 数値の検査は形を見ないので、 違う形でも通る (実測: 折れ線が閉じて ✓ が三角になっても墨の面積比は「読める」 だった)。 紙の大きさの crop は読めるか (小さすぎないか) を、 高い倍率の crop は形を言う。 作った側の「crop で確認した」 は、 どの倍率で何を見たかまで読んでから受け取る。
