...
- <a id="staging-window-race"></a>**明示 add でも「同じ file」 なら巻き込む** (2026-09-09 追加) — 直前の防御 (= 自分が編集した file を明示列挙) が効くのは **相手が別 file を触っている**ときだけ。 同一 file を 2 session が編集していると、 相手が `git add <その file>` を打った瞬間に **自分の未 commit hunk も一緒に staging される** (= git の staging は file 単位で、 hunk の作者を区別しない)。 実例 (2026-09-09、 同一 session 内で**双方向に 1 回ずつ**発生): session X が `TODO.yaml` に 2 行足す → 検証 (parse + 出力の before/after 確認) に数分 → その間に session Y が同 file の別 entry を直して明示 add + commit → X の 2 行が Y の commit message (「別件の手順訂正」) の中に入って push され、 X が commit しようとした時には `git status` が clean になっていた。 **内容は失われない**が、 ① commit message が中身を説明しない ② `git log -S` でしか「いつ入ったか」 を追えない ③ **public repo では相手の未検査変更が leak gate を素通りして publish される** (= 最も危険な帰結。 `git diff` を目で通す規律は「自分が書いた hunk」 を前提にしている)。
  ...
  - <a id="temp-index-commit"></a>**防御 1' = 同じ file に相手の未 commit hunk が居るときは、 一時 index から自分の分だけを commit する** (2026-09-22 追加、 実測 2 回) — worktree を切らずに済み、 git-crypt の repo でも smudge の罠が無い。 作業 tree は触らない (= 相手の hunk はそのまま残る)。 手順: ① `git show HEAD:<path>` を取り出し、 自分の変更だけを当てた版を作る (= 自分の置換を HEAD 版にもう一度当てる。 old 文字列の出現数を assert) ② `blob=$(git hash-object -w <その版>)` ③ `export GIT_INDEX_FILE=<一時 file>; git read-tree HEAD; git update-index --cacheinfo 100644,$blob,<path>` ④ **pre-commit hook を手で回す** (`.git/hooks/pre-commit` は `GIT_INDEX_FILE` の index を見る = `git diff --cached` が自分の分だけ) ⑤ `tree=$(git write-tree); unset GIT_INDEX_FILE; commit=$(git commit-tree $tree -p HEAD -F <msg>); git update-ref HEAD $commit` ⑥ `git reset -q -- <path>` で実 index の entry を新 HEAD に揃える (作業 tree は不変) → push。 ⚠️ commit-tree は hook を呼ばないので ④ を省くと gate を素通りする。 post-commit 系 hook も走らない。 ⚠️ 対になる file (例: 縮退の本体と archive) は同じ一時 index に両方入れて 1 commit にする。 ⚠️ 相手側は「作業 tree = HEAD + 相手の hunk だけ」 になるので、 相手の `git diff` からは自分の分が消えて見える (= 混乱しないよう相手に 1 行知らせる)。
