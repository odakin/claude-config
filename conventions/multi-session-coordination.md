...
- <a id="concurrent-fetch-ref-lock"></a>**同じ repo を 2 process が同時に fetch すると片方が即失敗する** (2026-09-12 追加) — `error: cannot lock ref 'refs/remotes/origin/<branch>': is at X but expected Y` (scratch で 6/6 回)。 相手の fetch が ref を更新した直後なので、 timeout (rc 124 / 137 / 143) と区別して 1 秒おいて 1 回だけ取り直せば通る。 これを「fetch 未完了 = timeout」 と報告すると原因を誤る。
