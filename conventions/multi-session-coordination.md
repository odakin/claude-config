...
- <a id="foreign-wip-scratch-worktree"></a>**live checkout に相手の未 commit 変更や未 push commit が居るときは、 自分の変更を origin から切った使い捨て worktree で commit・push する** (2026-09-12 追加) — `git fetch && git worktree add --detach <tmp> origin/<branch>` → 変更 → 検査 → commit → push 直前に再 fetch して rebase → `git push origin HEAD:<branch>` → `git worktree remove <tmp>`。 live の tree には触らない (= pull も autostash も起きない、 [#autostash-foreign-wip](#autostash-foreign-wip) の迂回路)。 3 点:
