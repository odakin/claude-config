...
塗り潰し marker の重心と境界線の傾きを data 座標で返す、`--selftest` は foil つき)。
...
⚠️ **枠の内側に文字がある図では、文字が marker と誤検出される**。script は充填率 (円盤 = π/4 ≈ 0.785) と
縦横比で落とすが、完全ではない — 返ってきた点の数が図の見た目と合うかを必ず目で確認する。
境界線 fit は残差が大きいとき `trustworthy: false` を返す (= 文字や網掛けを拾っている)。
**この警告が出た fit の傾きは使わない** — 数値が返ってくること自体は正しさの証拠ではない。
