...
11. <a id="python-file-long-nonascii-line"></a>非 ASCII を含む**長い行**の python script を file 経由で走らせない (= zsh でなく macOS 標準 python3 の罠、 本 family に置くのは「file に逃がした結果」 の罠だから)。 `python3 <file>.py` は、 **非 ASCII を含む 1 行が約 1 KB を超える**と偽の `SyntaxError: Non-UTF-8 code starting with '\xe3' … but no encoding declared` で落ちる。 file 自体は正しい UTF-8 で、 **同じ bytes を `compile()` に渡すと通る**ので「encoding が壊れた」 と読むと迷走する。 実測 (2026-09-12、 system python 3.9.6、 二分探索): 1 行 **1014 bytes まで OK / 1017 bytes で FAIL**、 ASCII だけなら 4000 文字でも OK、 先頭に `# -*- coding: utf-8 -*-` を置けば通る、 heredoc (`python3 - <<'PY'`) では起きない。 (境界が ~1 KB なので読み取り buffer が multibyte 文字を割っている、 と読めるが機構そのものは未確認 = 説明であって観察ではない。) ⚠️ **長い Bash command を file に逃がす**規律 ([`claude-code-permissions.md#long-command-falls-back-to-ask`](claude-code-permissions.md#long-command-falls-back-to-ask)) に従うと、 日本語の長い散文を含む patch script はまさにこの形になる = **一方の規律の remedy が他方の罠に入る**。 → 日本語を含む patch script を file に書くなら coding cookie を先頭に置く。
