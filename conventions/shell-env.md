...
→ **規則**:
1. 複数語を 1 変数に入れて分割に頼らない。 zsh で分割するなら `${=var}`、 そうでなければ配列にする。 対象を列挙して走査する検査は python (`subprocess.run(["git", "-C", path, …])`) で書く。 コマンドの出力を行ごとに回すなら `cmd | while read -r x; do …; done` (変数に溜めてから `for x in $v` しない)。 6・7 件目 (2026-09-12、 CI の完了待ちの使い捨て loop): `for pair in "repo sha" …; do set -- $pair; cd ~/Claude/$1` (2 件目と同形 → `cd` 失敗・sha の filter が空になり全 run を待った) と `ids=$(gh run list … -q …); for id in $ids` (2 つの ID が 1 引数 → `gh run watch` が失敗) を同じ session で書いた。 どちらも「結果が出た」 形で終わるので、 出力が対象 (repo・sha・件数) と合っているかを読むまで気付かない = 規則 2 の「何を走査したかを印字」 が効いた。
...
7. <a id="test-gate-no-pipe"></a>test を通ったら commit する chain で、 test を pipe に通さない。 `bash x.test.sh | tail -3 && git commit …` の終了値は `tail` のものなので、 test が落ちても commit が走る。 2026-09-12: 漏れ検査 hook の自己 test が 3 件落ちていたのに commit・push し、 CI が赤くなった。 出力は file に落として `rc=$?` で見るか、 `set -o pipefail` を先に置く。
