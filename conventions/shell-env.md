...
8. <a id="test-gate-no-pipe"></a>test を通ったら commit する chain で、 test を pipe に通さない。 `bash x.test.sh | tail -3 && git commit …` の終了値は `tail` のものなので、 test が落ちても commit が走る。 2026-09-12: 漏れ検査 hook の自己 test が 3 件落ちていたのに commit・push し、 CI が赤くなった。 出力は file に落として `rc=$?` で見るか、 `set -o pipefail` を先に置く。
