...
→ **規則**:
1. 複数語を 1 変数に入れて分割に頼らない (command の位置も同じ: `B="python3 x.py --flag"; $B sub` は zsh では 1 語の command 名になり `command not found`、 実測 = 配列 `B=(python3 x.py --flag); $B sub` にする)。 zsh で分割するなら `${=var}`、 そうでなければ配列にする。 対象を列挙して走査する検査は python (`subprocess.run(["git", "-C", path, …])`) で書く。 コマンドの出力を行ごとに回すなら `cmd | while read -r x; do …; done` (変数に溜めてから `for x in $v` しない)。 zsh 5.9 の実測: `for x in $(cmd)` と直接書けば分割されるが、 `v=$(cmd); for x in $v` は分割されない (出力全体が 1 語)。 コマンドの引数 (`printf '%s\n' $v`) も分割されない。 6・7 件目 (2026-09-12、 CI の完了待ちの使い捨て loop): `for pair in "repo sha" …; do set -- $pair; cd ~/Claude/$1` (2 件目と同形 → `cd` 失敗・sha の filter が空になり全 run を待った) と `ids=$(gh run list … -q …); for id in $ids` (2 つの ID が 1 引数 → `gh run watch` が失敗) を同じ session で書いた。 どちらも「結果が出た」 形で終わるので、 出力が対象 (repo・sha・件数) と合っているかを読むまで気付かない = 規則 2 の「何を走査したかを印字」 が効いた。 → **2026-09-12 に機械化** = [`hooks/zsh-word-split-guard.py`](../hooks/zsh-word-split-guard.py) (PreToolUse Bash、 実行 shell が zsh のとき `for … in $V` / `set -- $V` / `-- $V` / `git … $V` を、 `$V` が複数語だと同じ command 内で見えるときだけ deny + 直し方を返す)。 校正 = 手元 transcript の Bash 18,036 件で 22 件止まり、 実例の誤用が 21 件・同名変数の再利用による誤検出が 1 件 (誤検出のコストは書き直し 1 回)。
2. 対象を解決する段 (`cd`、 path・commit の存在) が失敗したら止める。 `cd` せず `git -C <path>` を使い、 解決できなければ `exit 1`。 使い捨ての worktree / 一時 dir に `cd` して試すときは特に `cd "$W" || exit 1` (実測: git-crypt の repo で worktree の作成が失敗し、 `;` で繋いだ `cd` も失敗して、 試験用の変更が live の checkout で走った。 worktree が落ちる理由 = [`multi-session-coordination.md#foreign-wip-scratch-worktree`](multi-session-coordination.md#foreign-wip-scratch-worktree))。 結果には**何を走査したか** (repo・commit・行数) を印字し、 対象ごとの行数が同じなどの不自然さを見る。
3. pattern を含む 1-liner は python で書く (1 件目の回避策)。
4. 変数の直後に `:` を続けるときは `${var}:` と書く (3 件目、 下記)。 非 ASCII の文字 (かな・漢字) を続けるときも `${var}` (UTF-8 の locale では zsh が文字を変数名に取り込む = [`shell-multibyte-truncation.md#unbraced-var-before-multibyte`](shell-multibyte-truncation.md#unbraced-var-before-multibyte))。
...
9. <a id="test-gate-no-pipe"></a>test を通ったら commit する chain で、 test を pipe に通さない。 `bash x.test.sh | tail -3 && git commit …` の終了値は `tail` のものなので、 test が落ちても commit が走る。 2026-09-12: 漏れ検査 hook の自己 test が 3 件落ちていたのに commit・push し、 CI が赤くなった。 出力は file に落として `rc=$?` で見るか、 `set -o pipefail` を先に置く。逆向きもある: 表示のためだけの `grep` を `&&` の途中に置くと、表示が 0 件なだけで後段の commit が走らず、`;` の後ろの出力だけが成功のように見える (2026-09-13、C locale で一致しなかった grep = [shell-multibyte-truncation.md#bracket-expression-c-locale](shell-multibyte-truncation.md#bracket-expression-c-locale))。表示は chain の外に置き、commit できたかは `git log -1` の hash で確かめる。`git commit … | tail -2 && git push` も同じ形で、並行 session の git 操作と `.git/index.lock` がぶつかって commit が失敗しても push は走る (2026-09-14: 自分の変更は working tree に残ったまま、相手の commit だけが push された)。commit の後は `git status --short -- <path>` で自分の file が消えたことを見てから push する。

10. <a id="unquoted-glob-nomatch"></a>コマンドの**引数**に書く glob も quote する (zsh の nomatch)。 zsh は glob が 1 つも一致しないと **そのコマンドを実行しない** (`(eval):1: no matches found: <pattern>`、 bash は pattern をそのまま渡す)。 踏みやすいのは file 名の glob でなく **flag の値**: `grep -rn --include=*.md pat .` は `*.md` に一致する file が cwd に無ければ **grep 自体が走らず**、 `;` で繋いだ前後の出力だけが残る (= 「検索したが 0 件」 と見分けがつかない)。 `&&` で繋いでいれば chain がそこで止まる (= 規則 7 と同じ mode)。 2026-09-12 に 2 回: `--include=*.md` と `ls -d ~/…/*<語>*`。 → glob は quote する (`--include='*.md'`)。 ⚠️ 失敗の痕跡は stderr の 1 行だけで、 見た目が「検索して 0 件」 と同じなので**不在の断定に直結する** — 層1 の検索 null nudge がこの形を拾う (その検出器が素の部分文字列 match で自己発火した件 = [`convention-design-principles.md#detector-fires-on-its-own-signal`](../docs/convention-design-principles.md#detector-fires-on-its-own-signal))。

11. <a id="python-file-long-nonascii-line"></a>非 ASCII を含む**長い行**の python を coding cookie 無しで走らせない (file でも heredoc でも。 = zsh でなく macOS 標準 python3 の罠、 本 family に置くのは「file に逃がした結果」 の罠だから)。 `python3 <file>.py` は、 **非 ASCII を含む 1 行が約 1 KB を超える**と偽の `SyntaxError: Non-UTF-8 code starting with '\xe3' … but no encoding declared` で落ちる。 file 自体は正しい UTF-8 で、 **同じ bytes を `compile()` に渡すと通る**ので「encoding が壊れた」 と読むと迷走する。 実測 (2026-09-12、 system python 3.9.6、 二分探索): 1 行 **1014 bytes まで OK / 1017 bytes で FAIL**、 ASCII だけなら 4000 文字でも OK、 先頭に `# -*- coding: utf-8 -*-` を置けば通る。 ⚠️ **heredoc (`python3 - <<'PY'`、 stdin) でも同じ閾値で起きる** (2026-09-13 実測、 同 python 3.9.6: stdin も file も 1 行 ~1023 bytes まで OK / ~1026 bytes で FAIL、 error は `in file <stdin>`、 cookie で両方通る。 前日の 1014/1017 と数 byte ずれるのは行の中身が違うため = 境界は「約 1 KB」 として扱う。 前版はここに「heredoc では起きない」 と書いていたが、 長い日本語の置換文を heredoc で流した patch が実際に落ちて反証された = 短い行でしか試していなかった)。 (境界が ~1 KB なので読み取り buffer が multibyte 文字を割っている、 と読めるが機構そのものは未確認 = 説明であって観察ではない。) ⚠️ **長い Bash command を file に逃がす**規律 ([`claude-code-permissions.md#long-command-falls-back-to-ask`](claude-code-permissions.md#long-command-falls-back-to-ask)) に従うと、 日本語の長い散文を含む patch script はまさにこの形になる = **一方の規律の remedy が他方の罠に入る**。 → 日本語を含む python は、 file に書くときも heredoc で流すときも **coding cookie を先頭に置く**。 **版依存** (2026-09-13 実測): 同じ machine の Homebrew python 3.14.4 は 20 KB の行まで落ちない = 罠は古い system python (3.9.6) のもので、 `python3` が PATH でどちらに解決するかで決まる。 python を更新したら [`scripts/probe-python-nonascii-line.py`](../scripts/probe-python-nonascii-line.py) `--python <path>` で測り直し、 数字か規則を直す。
12. <a id="bash-tool-grep-ignores-gitignore"></a>網羅を主張する検索に、Bash tool の素の `grep -r` を使わない。Claude Code の shell snapshot は `grep` を shell 関数として定義し、Claude の実行 file を `ugrep` として `-G --ignore-files --hidden -I --exclude-dir=.git …` 付きで起動する (`whence -f grep` で確かめられる)。`--ignore-files` は `.gitignore` を読むので、**ignore された path は再帰検索から黙って外れる**。出力の path に `./` が付かないのが見分ける手がかり。2026-09-13: 親 repo が `external/` を ignore し、その下に原稿の別 clone を置いた構成で、`grep -rl` の結果から clone 内の file だけが抜けた (同じ検索で関数経由 4 件、`command grep -rl` 8 件)。0 件ではなく一部が抜けるだけなので、検索 null を見張る hook も鳴らない。網羅には関数を迂回する `command grep -r` か `/usr/bin/grep -r` を使う。`-I` で binary も飛ばされるので、走査した範囲は結果と一緒に書く。 **pipe で渡した binary も同じ**: `head -c 9 f | grep -q GITCRYPT` (magic header の判定) は関数経由だと rc 1、 `command grep -q` と script の中の bash では rc 0 = Claude が手で確かめた結果だけが「一致しない」 側に倒れる (実測、 [`hook-authoring.md#locked-engine-interpreter-rc`](hook-authoring.md#locked-engine-interpreter-rc))。
13. <a id="pipefail-expected-failure-grep"></a>**終了値 1 を期待するコマンドの出力を `| grep -q` で確かめない** (`set -o pipefail` 下)。 pipeline の終了値は「どれかが非 0 なら非 0」 なので、 grep が一致しても前段の exit 1 で偽になる (実測: `--check` が finding を出して exit 1 する検査の出力を `| grep -q '<期待文>'` で確かめた test が、 正しい出力なのに落ちた)。 出力を変数に取ってから grep する: `out="$(cmd 2>&1)"; echo "$out" | grep -q …`。 規則 9 の逆向き (失敗を隠す pipe) と対。
14. <a id="zsh-log-builtin"></a>macOS の unified log は **`/usr/bin/log show`** と full path で書く。 zsh では素の `log` が組み込み (login の監視用) に取られ、 `log show ...` は `log: too many arguments` で失敗する (bash では起きない)。 層1 の規約・script で 3 度書かれていた罠 (= [`macos-app-crash-triage.md`](macos-app-crash-triage.md) / [`macos-claude-app-notifications.md`](macos-claude-app-notifications.md) / [`macos-exec-policy-kill.md#log-reading`](macos-exec-policy-kill.md#log-reading)) をここに集めた。 script から呼ぶときも full path にする。
15. <a id="zsh-pipestatus-lowercase"></a>pipeline の各段の終了値は zsh では **`$pipestatus`** (小文字、 添字は 1 始まり)。 bash の `${PIPESTATUS[0]}` は zsh では空で、 `[ "${PIPESTATUS[0]}" -eq 0 ]` のような検査は常に成立して失敗を通す (実測)。 規則 9 と同じ mode (失敗が成功に見える) なので、 前段の終了値が要る chain は `set -o pipefail` か出力を file に落として `rc=$?` で見る。
16. <a id="no-stacked-heredocs"></a>1 つの command に heredoc を 2 つ重ねない (`python3 - <<'PY' … PY` の後に `--summary "$(cat <<'MSG' … MSG)"` を続ける形)。 2 つ目の本文が 1 つ目の program に読まれて壊れた (実測、 再現条件は未確定)。 長い文面・code は先に file に書き (Write tool)、 `"$(cat <file>)"` で渡す = 長い command を止める guard (3000 字) にも掛からない。
...
**背景 (2026-09-12)**: [`scripts/repo-sync-sweep.sh`](../scripts/repo-sync-sweep.sh) の「stash が別 process に pop された」 案内が short sha を埋めており、 CI で 1 回だけ `FAIL: 比較手順が使えない (sha='1168850')`。 再実行で緑になるので flaky に見えた

<a id="stash-show-no-pathspec"></a>**同じ族の近縁 — `git stash show` は path を取らない**: `git stash show -p 'stash@{0}' -- <path>` は `--` の後ろも revision と読み `Too many revisions specified` で rc 1 (実測 git 2.39)。 stash の 1 file だけを見るなら、 stash の commit とその親の diff にする: `git diff 'stash@{0}^1' 'stash@{0}' -- <path>` (未追跡 file を含めた stash の分は `'stash@{0}^3'` 側)。 stash を drop する前に中身を file 単位で確かめる時に踏むが、 実体は **3.7% で user に壊れた手順を渡していた**バグだった。
