...
- **サイズの決め方** (= 2026-07-27 の 4 周の教訓): 写真からの推定は **±25% ブレて収束しない** (押し圧で印影外周の写り方が変わる、 実測 24〜35pt に散った)。 **唯一の真値 = ハンコ印面の物理径を定規で測る** (mm ÷ 0.3528 = pt)。 採用値は**書類種ごとに SoT へ固定値として記録**し、 新しい様式では最初の 1 回だけ試し刷り並置で持ち主確認 → 以後その値。
