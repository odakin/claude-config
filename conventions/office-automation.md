...
| **刷った PDF から様式番号・区分の枠・㊞ の図形が消えた** (案件の xlsx には在る / 様式番号の末尾 1 字だけ欠ける) | PDF 用の temp を openpyxl で保存した (xlsx の走査に出ない) / 字幅ぎりぎりの枠の clip | 図形を移し直す or Excel で刷る。 検出 = `check-form-static-text.py` → [`drawings-lost-in-temp-pdf`](#drawings-lost-in-temp-pdf) |
