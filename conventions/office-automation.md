...
| `save` だけが `-50` で落ちる (値の書き込みは通る) / PDF 書き出しが**成功と返るのにファイルが無い** / Word・Excel 上部に「表示のみ … ライセンス認証」 の帯 | **Office のライセンスが未認証 (表示のみモード)** = 開けるが保存・書き出しができない。 新しい Mac への移行直後に起きやすい (実測) | 認証の有無 = `~/Library/Group Containers/UBF8T346G9.Office/Licenses/<版>/` が空か (認証すると file ができる)。 サインインは user 操作 (資格情報を agent が扱わない)。 待つ間は同じ folder に file ができるのを見張って続きへ進む (「済んだら教えて」 と頼まない)。 一度 Excel で保存・書き出しできる機械かを確かめる = staging の e2e 検査 ([`office-pregranted-staging-dir`](#office-pregranted-staging-dir)) |
...
2. **drawing XML を migration** (= openpyxl save 後の xlsx に、 元 xlsx の `xl/drawings/` + 関連 `_rels` part を zip レベルでコピーし直す)。 値編集と drawing 保護を両立したいが Excel を起動できない (CI 等) とき。 ⚠️ **別 file (= 標題を持つ blank テンプレ) から注入して復元するときは注入前に 4 点を確認** (= いずれか欠くと標題ずれ / 破損): ① 両 file の **merged 範囲が一致** (= drawing の anchor cell がずれず標題が正位置に乗る保証)、 ② テンプレ側 drawing が **standalone** (= `xl/media` 画像を参照しない。 参照ありなら media と rels も連れて行かないと dangling rel になる。 連れて行く場合は `[Content_Types].xml` に拡張子 Default 〔例: emf〕 も追加)、 ③ 注入先に **drawing が皆無で `rId` が衝突しない** (= 既に drawing を持つ file に重ねると rId 重複で Excel が破損判定)、 ④ **`<drawing r:id="…"/>` を挿す worksheet root に `xmlns:r` が宣言されているか確認** (= 一部の生成 tool 産 xlsx は root が `xmlns` のみで **`xmlns:r` 無し** 〔その file の `<legacyDrawing>` が inline `xmlns:r` を持っているのが signal〕。 そのまま挿すと unbound prefix = invalid XML で Excel が「壊れている」 ダイアログを出す。 inline で `<drawing xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" r:id="…"/>` と書けば安全)。 **注入後の機械 gate**: `scripts/check-xlsx-integrity.py FILE.xlsx` (= `check-docx-integrity.py` の xlsx 版。 全 XML well-formedness 〔unbound prefix = ④〕 + rels 両方向参照整合 〔dangling Target / 未定義 rId = ③ の検出面 / rId 重複〕 + [Content_Types] coverage を Excel 不要・決定論で検出、 exit 1 で fail)。 **zip 直編集した xlsx は納品前に必ず通す** — ③④ は実害 2 件とも本 gate で捕捉できた class (= 規約 prose の注意書きだけでは 2 回とも素通りした、 2026-06-10 RCA)。 pass 後 [`pdf-visual-confirm`](#pdf-visual-confirm) で標題の有無・位置を目視。 最終 ground truth は実機 open (= docx の教訓と同じ、 validator は必要条件。 ⚠️ AppleScript `open` の戻り値 "opened" は修復ダイアログの有無を検出できない = open 成功を健全性の根拠にしない)。
   ...
   **再利用できる実装** = [`scripts/formcase/drawings.py`](../scripts/formcase/drawings.py) の `graft_drawings(src, dst, drop=None)` (同じ workbook を openpyxl で読んで save した file に、 ①〜④ を満たして同名 sheet の図形を移し直し、 本 gate まで通す。 form control は移さない。 formcase の build が temp の保存ごとに呼ぶ = [`form-case-pipeline.md#drawings-survive-temp`](form-case-pipeline.md#drawings-survive-temp))。
...
**事後の横断 sweep**: この罠を一度踏んでいたと発覚したら、 **repo を跨いで、 同じ様式・同じ機構 (openpyxl の保存) の全経路**の過去の openpyxl 製 xlsx を drawing 数で、 その経路で刷った PDF を雛形との照合で走査する (= 下の [`#cross-repo-sweep`](#cross-repo-sweep)) (= 1 件見つかった時点で同経路の他 file も喪失している可能性が高い。 2026-06-12 の走査では 3 file 中 3 file が喪失済みだった)。
...
- ✅ **日付固定の配当 = 「先刷り・先押印・後提出」 pattern**: 提出タイミングが規制される書類 (= 報告書は用務後にしか出せない 等) でも、 **規制されているのは提出であって印刷・押印ではない**。 日付を「あるべき日」 (= 帰着日等) で固定してあれば、 紙は事前に刷って**関係者の押印を 1 回で全部集められる** (= 用務後に本人 〔学生・学外者〕 を捕まえる手間の設計排除)。 提出フェーズ別に PDF を分割出力しておくと「束のまま誤提出」 ([`shucchou-runbook` 型の 2 フェーズ表] 参照) も防げる。
...
- 🔥 **乗算合成が本物らしさの核心** (= 2026-07-27 持ち主の洞察「線を完全に消しちゃうと不自然になるんだ。 下の黒い線も透けて見えてるよね？ 正しいのだと」): **実物の朱肉は半透明**で、 黒い罫線・印字は印影の**下から透けて見える** (= 減法混色)。 PNG を不透明で貼ると下の線・文字を**消して**しまい、 それが印刷印影の最大の giveaway。 engine は PNG を直接貼らず、 **対象領域の下地 (罫線・文字込み) を高 DPI render → 印影と乗算 (`out = base × lerp(1, seal_rgb/255, α)`) → 合成画像を貼り戻す**。 黒×赤 ≈ 黒 (線が透ける)、 白×赤 = 赤。 合成は下地込みなので継ぎ目も原理上出ない。
- **配置**: 乗算合成後は**線またぎが自然に見える** (= 実物と同じ) ので、 罫線回避 `avoid=h` は不要になった (default `none`)。 使うのは「金額など読めなければならない内容を覆いそうな時」 だけ。 サイズ優先の原則は不変 = **実物と同じ径をどの書類でも使う** (小径に絞るのは「実物と違うサイズ」 という別の giveaway)。
- **サイズの決め方 = 層1 [`hanko-digitization.md#seal-size-determination`](hanko-digitization.md#seal-size-determination) が正本** (= 規格径 → 円ターゲット較正 → fill 補正 → 並置目視 の 4 段)。 🔥 特に **PNG の content fill 系統誤差** (= canvas 余白のせいで紙上の印影が指定 × fill にしか出ない、 実測 11% 過小で校正 loop が 5 周迷走) — engine が PNG の不透明 bbox から自動補正するので **指定 pt = 紙上インク径**。 採用値は**書類種ごとに SoT へ固定値として記録**。
...
- **判定軸は 2 つ**: ① その queue で化けた実績があるか ② 記号・数式が多い文書か (= 数式フォントは本文フォントより glyph の当たりが薄い)。 どちらかが当たれば raster を既定に
...
**Excel / Word / PowerPoint に file を開かせる・保存させるのは、 wrapper でも手書きの osascript でも案件ごとの driver でも staging root の内側だけ** (2026-09-15 に機械で強制)。 文章だけの規則では、 staging 導入 (2026-08-21) 以前に書かれた案件ごとの driver を写した session が in-place で開き直し、 dialog が繰り返し出た (実測) — prompt に書かなくても既定で正しくなるよう、 次で固定する:

- **やり方 (= 楽な方が正しい)**: PDF 化 / 画像貼付は wrapper (`xlsx-to-pdf.sh` / `docx-to-pdf.sh` / `pptx-to-pdf.sh` / `affix-image-xlsx.py`)。 手書きの osascript は [`scripts/office-stage-run.sh`](../scripts/office-stage-run.sh) `<file> -- osascript fill.applescript {}` (`{}` = staged copy、 成功時に書き戻し。 出力は `{dir}/out.pdf` に書かせて `--out out.pdf=<dest>`、 AppleScript 側は path を literal で書かず `on run argv` で受ける)。 **新しく書く案件ごとの driver は helper を使う** (python = `from office_staging import Stage`、 bash = `source …/scripts/lib/office-staging.sh` + `office_stage_file`)。 旧い driver を写すときも、 まず export / 記入の部分をこの形に直す。
- **強制**: PreToolUse(Bash) hook [`hooks/office-inplace-guard.py`](../hooks/office-inplace-guard.py) が、 (a) inline osascript (heredoc 含む) で開く・保存する path の literal が staging root の外、 (b) `osascript <file>.applescript|.scpt` で script 内の literal か argv の path が外、 (c) Office を osascript / appscript / JXA で open・save する python / shell script (直接実行・`-c`・heredoc 含む) が helper を import / source していない、 (d) wrapper の `--no-stage` や `CLAUDE_OFFICE_STAGING=0`、 (e) Office app を名指しで止める command (`killall` / `pkill` / `kill $(pgrep …)` / inline osascript の quit = [`office-app-reset-guard`](#office-app-reset-guard)) を **deny** して上の直し方を返す。 通すもの = wrapper / `office-stage-run.sh` 配下 / staging root (と `CLAUDE_OFFICE_STAGING_DIR`) の内側 / 判定できない入力 (未解決の変数・読めない file = fail-open)。 配線 = [`hooks/settings-entries.json`](../hooks/settings-entries.json) (= 各マシンの `sync-hook-settings.sh` が次の session 開始 / pull で入れる)、 実効性 = `office-inplace-guard.py --canary` (本番の settings と install 済み hook がカナリアを deny するか = `ARMED` / `NOT ARMED` / `対象外`)。
...
- ⚠️ **同じ発行元の様式でも有無が割れる**: 実測で、 同一機関の出張・招聘様式 7 種のうち **5 種は CF ルールを持ち 2 種は 0 件**だった (= 該当セル数も 5-23 とばらつく)。 さらに `page_setup.blackAndWhite` が `True` の様式は色が紙に出ない。 ∴ **「隣の様式では問題にならなかった」 は前例にならない** (= 様式ごとに CF 有無 × blackAndWhite 有無 で 4 通り)。
...
- <a id="seal-image-detected-despite-craft"></a>**画像の作り込みでは解決しない (実測)**: 実物の押印を撮った写真から作った印影・印刷の色の校正・押すたびに変える変奏のランダム選択まで揃えた画像でも、 紙の窓口は「印刷の形式」 と見分けた。 見分けられた書類は窓口へ出向いて隣に押し直すことになり、 画像で出した書類の数だけ手戻りが一度に来る。
- <a id="hearsay-acceptance-is-not-policy"></a>**「画像でも通る」 を運用の根拠にしない**: 同僚の経験談・一度通ったという観察・窓口が何も言わなかったことは、 窓口の方針ではない (黙認は後で覆る)。 画像を運用にするのは窓口自身が書面で明示した時だけ。 明示が無いまま画像で出すなら、 覆った時に押し直す書類の一覧を出せるようにしておく (提出の記録に「画像で押した」 を残す)。
- **機械で押さない側に倒す形**: 様式 pipeline は印影を重ねずに押す頁と欄を列挙する ([`form-case-pipeline.md#seal-mode`](form-case-pipeline.md#seal-mode) の `physical`)、 印影の画像を引く道具は既定で止める、 メール添付の gate は例外なしで止める ([`#seal-artifact-marker`](#seal-artifact-marker) の代理印刷の例外を設定で閉じる)。
...
### <a id="cell-address-rule-rot"></a>🔥 記入指示をセル番地で書くと、 様式の組み替えを生き延びて**別の意味の行**に書く

**症状**: 手順 doc に「`T52` と `T64` を〈固定値〉で上書きする」 と書いた指示が、 その後の**表の組み替え** (= 1 行の意味を変える変更。 例: 「1 行 = 1 行程」 の表を「1 行 = 1 日」 に改めた) を生き延び、 番地が**別の意味の行**を指すようになった。 誰も嘘をついておらず、 指示どおりに書くと**中日の行に前の設計の値が残る**。 気づいたのは提出・送付の後 (実測)。

**なぜ番地は腐るのに気づかれないか**: 番地は「その様式のどの行か」 ではなく「格子のどこか」 しか意味しない。 表の行数・block の粒度・雛形の改版は格子を動かすが、 **散文の番地は動かない**。 しかも書いた値そのものは正しい (= 値検査・合計の検算・見た目の gate を素通りする。 [`label-overwrite-bug`](#label-overwrite-bug) は「別の cell に書いた」 が検出できる形、 本節は「正しい形式の値を正しくない行に書いた」)。

**規律**:

1. **規則は構造に相対で書く** — 「最初の使用 block」「最後の使用 block」「〈見出し〉の下の n 番目」「未使用 block すべて」。 番地を書くなら、 その番地を**機械照合される 1 か所** (記入 map / spec) にだけ置き、 手順 doc は規則 id で引く ([`convention-design-principles.md#rule-copy-lint-reach`](../docs/convention-design-principles.md#rule-copy-lint-reach))。
2. **番地の第二の写しを gate に持たない** — 検査 script が block の anchor を hard-code すると、 spec を直しても検査が旧番地で裁く (= 直した側が FAIL になる)。 anchor は spec から読む。
3. **layout の定数 (行高・印刷範囲・改ページ) は配布雛形から導出する** — 前に提出した file から写した定数は、 その案件の値の長さに合わせて人が動かした結果を含む ([`template-base-not-precedent-base`](#template-base-not-precedent-base) の layout 版)。 雛形 → 様式の spec の明示的な上書き (理由つき) → 記入値の長さ、 の 3 段だけを出どころにする。
4. **表の粒度を変えたら、 番地を書いた文を grep する** — 変更の commit に「番地 (`[A-Z]+[0-9]+`) を含む手順 doc の行の棚卸し」 を含める。 これは規律 1 が行き渡るまでの過渡の網。
...
| 行の下半分が空白 | 使わなかった行の高さが雛形の初期値のまま | 不可視 |
...
- <a id="gate-owns-correctness-not-the-estimate"></a>🔥 **FAIL でも出せる生成器は、 検査を無視する習慣を教える**: gate を「回すが出力は作る」 (= 警告つき export / `--force` / skip flag) にすると、 FAIL は読まれない 1 行になり、 壊れた成果物が紙とメールに出ていく。 **書かない・理由を言う**を既定にする (= 出力 0 file。 「何も出ない」 は強い信号で、 かつ次の行動が一意に決まる)。
- <a id="retry-belongs-inside-the-tool"></a>🔥 **直せる FAIL の直し方は道具の内側に置く** — 「gate が落ちたら人が値を詰めて叩き直す」 にせず、 **落ちた欄を道具が自分で広げて作り直す loop** (例: 欄に収まらなかった値を折り返し → 行を伸ばして再描画、 上限回数つき) を build の中に持ち、 **上限まで直らなければ出力を書かずに止める**。 これが効く理由は 2 つ: ① 人の手を待つ間に「1 回くらい目で見て OK にする」 迂回路ができない ② **見積りの精度が load-bearing でなくなる** — 事前の見積り (字幅・行の送り・列幅の換算) は所詮近似で、 精度を上げても外れる日が来る。 見積りは**刷り直しの回数を減らすだけ**で、 紙の正しさは gate が持つ、 という分担にすると、 見積りの定数は「外れたら遅くなる」 だけの調整値に落ちる (= 壊れ方が silent でなくなる)。 ⚠️ **悪化する方向の FAIL は再試行しない** — 縮小で収めた結果「字が小さすぎる」 に落ちた類は、 広げ直すと元の FAIL に戻るので即止める (再試行の条件を「この直しでその FAIL が消える」 に限る)。
...
1. **機械**: [`scripts/pdf-print-preflight.py`](../scripts/pdf-print-preflight.py) `<print.pdf> --template <雛形.pdf>` (or `--expect-pages N`) が PASS (= page 数一致 + PyMuPDF 描画 / 非埋め込み font ゼロ + 頁の役割 = 全頁が提出頁の宣言つき、 または宣言の無い 1 頁で出さない頁に見えない)。 FAIL なら `--rasterize <out.pdf>` で **RGB 600dpi raster** を作り、 そちらを刷る (認印の朱色は RGB でしか残らない)。 頁を選ぶ時は `--pages <頁>` を足す (選んだ頁だけの file を作り、 提出頁と宣言する)。 ⚠️ `--template` は雛形の全頁数と比べるので、 提出頁だけを刷る file には `--expect-pages <提出頁の数>` を使う。 ⚠️⚠️ **PASS を「raster 化は不要」 と読まない** — font 検査は font の出自しか見ておらず、 RIP が glyph を出すかは見ていない。 記号・数式の多い文書と、 その queue で化けた実績のある文書は PASS でも `--rasterize` を既定にし、 **部数のある刷りは 1 部出して現物を見てから残りを出す** ([`print-raster-pdf`](#print-raster-pdf) の「射程は『加工した PDF』 より広い」)。
...
5. **用紙**: `lp -o media=A4` (や PageSize) は**本体の用紙設定を上書きする保証にならない** — 本体 (操作パネル) の用紙サイズが別サイズのままだと、 A4 指定の job がその紙に刷られた (実測)。 刷る前に「本体の用紙サイズ設定」 と「トレイの紙」 の両方を user に確かめてもらう。 ⚠️ 違うサイズで出たと言われたら、 原因を推測で説明しない (= 「プリンタは紙を見ない」 等の未検証の断定をしない)。 本体の設定を見てもらうのが先。
   - <a id="printer-media-ipp-step"></a>**先に機械で本体に聞く** (追加の段): `pdf-print-preflight.py <print.pdf> --printer <queue>` が本体のトレイの用紙を IPP で読み、 PDF の寸法と合わなければ 🔴 (lp に掛かる hook も同じ問い合わせをして止める)。 ⚠️ 一致しても上の user への確認は省かない — 本体の報告は設定であって紙の実測ではなく、 設定を変えたときに報告が追従するかも未検証 ([本体の用紙を IPP で聞く](#printer-media-ipp))。
...
<a id="headless-orphan-blocks-gui-launch"></a>⚠️ **残った headless の browser は、 本人がその browser を開けなくする** (macOS、 実測): headless の process もその app として LaunchServices に登録される (`lsappinfo find bundleid=<id>` で見える、 type = BackgroundOnly)。 残っていると、 本人が Dock やアプリ一覧から開いても macOS は「もう起動している」 として窓の無い headless を前に出すだけで、 何も起きない (crash のダイアログも出ない)。 残る経路は「止める処理を呼び元が持っている」 設計そのもの = 呼び元が SIGKILL (呼び元の timeout・中断) で消えると `finally` は走らず、 `start_new_session` で別 group にした browser だけが残る。 **止める役を呼び元と別の process に持たせる** (呼び元の pid を見張り、 消えたら browser の group を止める) + **起動前に前回の残り (親が消えた自分の印の headless) を止める**。 test は「呼び元を SIGKILL しても偽の browser が数秒で消える」 と、 見張りを外した実装でその test が落ちること。 実装 = 同 script の `start_watchdog` / `sweep_stale_renders`。 症状から入った場合の切り分け = [`macos-app-crash-triage.md#launch-does-nothing`](macos-app-crash-triage.md#launch-does-nothing)。
...
2. **刷る file は提出頁だけで作り、 その宣言を file に書き込む**: 頁は file で選ぶ (`lp -o page-ranges` は無視される queue がある = [`lp-page-ranges-distrust`](#lp-page-ranges-distrust))。 印刷用の raster 版は字を持たず、 刷る段で中身を読めない → 作る道具が「全頁 = 提出頁」 の宣言を PDF に書き込み (Info 辞書。 raster 化・頁の抜き出しで引き継ぐ)、 刷る段がそれを読む。 実装 = [`scripts/lib/print_pages.py`](../scripts/lib/print_pages.py) (宣言・推定・判定。 記入 map を持つ生成道具には `role_map_problems` = 役割の宣言の矛盾 / `anchor_misses` = 頁の目印の照合 / `declare_submit` = 出力への宣言、 宣言の無い頁に strong の推定が当たれば書かずに止める理由を返す) + [`scripts/pdf-print-preflight.py`](../scripts/pdf-print-preflight.py) `--pages <頁>` (その頁だけの raster / vector を作って宣言する)。
...
<a id="all-pages-are-deliverable"></a>**全頁とも出す物のときも宣言は要る** (= 様式でない配布物): 束のすべてが渡す物 (配布資料・手引き・読むために刷る資料) でも、 gate は「宣言の無い 2 頁以上」 で止まる。 これは誤検出ではなく、 **頁の一覧を 1 度見て「全部出す」 と決める**ことが gate の目的だから (出さない頁が 1 頁も無い、 を確かめた記録が宣言)。 決めたら `--pages all` で宣言する。 頁を選ぶだけなら `--extract` (= vector のまま。 字が残り file も軽い)。 ⚠️ **ただし font の検査の ✓ を「raster 化は不要」 の判定に使わない** — ✓ は「[`print-raster-pdf`](#print-raster-pdf) の既知の壊れ方 (PyMuPDF 描画・非埋め込み) が無い」 だけで、 **RIP が全 glyph を出す保証ではない**: 正しく subset 埋め込みされた CM Type1 CFF + 和文 CID の束が font ✓ のまま laser queue で記号 (slash・Greek 大文字) を落とした実測がある。 ∴ **記号・数式の多い文書と、 その queue で化けた実績のある文書は `--rasterize` を既定**にし、 `--extract` で出すのは刷った現物を見て確かめられるときに限る。 gate の対処文も同じ向きに揃えてある (以前は `--rasterize` しか案内しておらず、 font ✓ の束まで raster 化に誘導していた → 頁の選び方と raster の要否を別の軸として書き分けた)。

**誤判定の向き**: 止めすぎ = 宣言を 1 回足す手間 / 見逃し = 紙の無駄 (gate の無い状態と同じ)。 ∴ 止めるのは strong の語と宣言の無い複数頁に限り、 weak は一覧に出す。 実測 (提出した様式・案内・要領・スライド・論文の束で試した): strong の語が当たったのは記入例・要領・手引・注意事項・白紙だけで、 提出した様式の頁には当たらなかった。 weak の「…について」 は案内文書・スライド・同意書の題にも当たり、 罫線なしの一覧は論文の目次や短い書状にも当たる (= だから weak)。 白紙の判定は「36 dpi で暗い画素が 2 個以下」 = 白紙の scan は 0、 短い 1 語だけの頁でも 10 を超える (実測) ので、 字の少ない頁を白紙と誤らない。 **推定の語を足す・外す前は同じ束で数え直す** = `python3 scripts/lib/print_pages.py --scan <PDF か dir ...> [--strong-only]` (疑わしい頁を 1 行ずつと集計を出す。 一般則 = [`convention-design-principles.md#green-check-endorses-unasked-premise`](../docs/convention-design-principles.md#green-check-endorses-unasked-premise))。
...
<a id="one-form-per-person-file"></a>**1 人 1 枚の様式を何人分か作るときは、 人ごとに別の file にする**: 何人分かを 1 つの PDF に並べると、 印刷機の既定が両面のとき 2 人目の様式が 1 人目の裏に刷られる (= 別人の書類が 1 枚の表裏になり、 別々に署名・提出できない)。 刷る人に片面指定を頼るのでなく、 file を分けて **1 file = 1 人の頁だけ**にしておけば、 既定が両面でも崩れない。 `--rasterize <人ごとの.pdf> --pages <その人の頁>` を人数分回す (実測)。

## <a id="single-gray-level"></a>グレーは 1 段階に固定する — 薄さは色でなく線幅で出す (2026-09-12)

印刷所・印刷機は**複数のグレー濃度を描き分けてくれるとは限らない**。下地 (罫線・補助線・参考要素) を本文から沈めたいとき、濃度を 2 段階 3 段階と刻むと、刷り上がりで潰れて区別が消える。∴ **濃度は文書全体で 1 段階**に決め、強弱は**線幅**で出す。

- **色と太さをマクロ 2 つに集約**しておく (= 試し刷りの後に触る場所が 2 行で済む)。文中に濃度をリテラルで散らすと、1 段階に揃っているかを目で確かめられなくなる
- **細線 × 淡色は掛け算で消える**: 0.3 pt 級の細い線を 30% 未満まで薄くすると、トナー・用紙・機械によって飛ぶ。薄くしたいときは **まず線幅、次に濃度**の順に動かし、最後は実物で確認する ([#print-preflight](#print-preflight))
- 画面で見えた薄さは印刷の薄さではない。薄さの判断は試し刷りまで確定させない

## <a id="printed-artifact-staleness"></a>印刷した紙は digital SoT の fork — 印刷後の修正は「物理伝播 event」

**構造**: 様式 xlsx/PDF を印刷した瞬間、 紙は digital file から**分岐した第二の実体**になる。 以後 digital 側をどれだけ直しても紙は勝手に直らないが、 version 管理・検証 script・diff gate は全部 digital しか見ないため、 **「直したから OK」 と「直した版が紙で提出される」 の間に誰も見ていない隙間**ができる。 典型の壊れ方: 印刷 → 差し戻し等で xlsx を修正 + PDF 再生成 (digital は完治) → 提出日に**手元の旧い紙**をそのまま提出 → 窓口から再差し戻し。 修正から提出まで日が空くほど、 また「印刷・押印は先に一括 / 提出は後」 型の運用ほど起きやすい。

**規律 3 点**:

1. **印刷は記録する event**: 刷った turn で、 当該 task の記録 (TODO notes 等) に**日付つきの点呼行**を残す (書式は運用側で固定、 例: `印刷版: YYYY-MM-DD (対象 = ...)` / `印刷版: なし(メール提出)` / `印刷版: 不明(要確認)`)。 日付が無い「印刷済」 の散文は staleness 判定に使えない。
