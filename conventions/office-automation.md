...
<a id="office-inplace-guard"></a>**規則 (機械で強制、 2026-09-15)**: **Excel / Word / PowerPoint に file を開かせる・保存させるのは、 どの経路でも staging root の内側だけ**。 文章だけの規則では、 staging 導入 (2026-08-21) 以前に書かれた案件ごとの driver を写した session が in-place で開き直し、 dialog が繰り返し出た (実測) — prompt に書かなくても既定で正しくなるよう、 次の 3 つで固定する:

- **やり方 (= 楽な方が正しい)**: PDF 化 / 画像貼付は wrapper (上記 4 本)。 手書きの osascript は [`scripts/office-stage-run.sh`](../scripts/office-stage-run.sh) `<file> -- osascript fill.applescript {}` (`{}` = staged copy、 成功時に書き戻し。 出力は `{dir}/out.pdf` に書かせて `--out out.pdf=<dest>`、 AppleScript 側は path を literal で書かず `on run argv` で受ける)。 **新しく書く案件ごとの driver は helper を使う** (python = `from office_staging import Stage`、 bash = `source …/scripts/lib/office-staging.sh` + `office_stage_file`)。 旧い driver を写すときも、 まず export / 記入の部分をこの形に直す。
