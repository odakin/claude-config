...
- 🚨 **配置の優先順位 = ① 実物と同じサイズ > ② 罫線・下線に重ねない** (= user 裁定 2026-07-27 2 段階: まず「線に重ねるな」、 次に小径版を見て「ちっちゃすぎ、 **サイズが優先**」)。 印刷印影が黒い罫線と交差すると印刷だと判別されやすい (実物押印は朱肉が線の**上に**乗って自然、 印刷は黒線がくっきり残る) ので、 engine の `avoid=h` (default on) が水平罫線との交差弦長を最小化する縦シフトを自動で選ぶ — **ただしサイズを縮めてまで回避しない**。 表の行高が印影より小さく回避不能な欄は `avoid=none` で**中央対称またぎ**にする (= またぎ幅が上下均等で最小 = 一番目立たない。 小径に絞るのは「実物と違うサイズ」 という別の giveaway を生む)。
