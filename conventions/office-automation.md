...
<a id="all-pages-are-deliverable"></a>**全頁とも出す物のときも宣言は要る** (= 様式でない配布物): 束のすべてが渡す物 (配布資料・手引き・読むために刷る資料) でも、 gate は「宣言の無い 2 頁以上」 で止まる。 これは誤検出ではなく、 **頁の一覧を 1 度見て「全部出す」 と決める**ことが gate の目的だから (出さない頁が 1 頁も無い、 を確かめた記録が宣言)。 決めたら `--pages all` で宣言する。 このとき **font の検査が ✓ なら `--rasterize` でなく `--extract`** (= vector のまま頁を選ぶ) — raster 化は font が 🔴 のときの対処であって、 字の残った PDF を無用に画像化すると file が重くなり、 字を選べない紙面専用の PDF になる。 ⚠️ gate の対処文が長く `--rasterize` しか案内していなかったため、 font が ✓ の束まで raster 化する形に誘導されていた (実測) → 対処文を `--extract` 起点に直し、 font が 🔴 のときだけ raster と書くようにした。
