<!-- doc-meta
when: Claude Code の permission prompt 削減・deny/ask/allow 設計を触るとき + tool call が拒否されて原因 (rule / hook / classifier) を切り分けるとき (#hook-masks-deny) + user が chat で「許可」 と言ったのに同じ call がまた拒否されたとき (#chat-consent-does-not-reach-classifier)
