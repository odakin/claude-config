<!-- doc-meta
when: Claude Code の permission prompt 削減・deny/ask/allow 設計を触るとき + tool call が拒否されて原因 (rule / hook / classifier) を切り分けるとき (#hook-masks-deny) + user が chat で「許可」 と言ったのに同じ call がまた拒否されたとき (#chat-consent-does-not-reach-classifier) + 本人の口座番号を、 本人が印刷して出す様式に印字するとき (#own-numbers-in-local-forms)
...
### <a id="own-numbers-in-local-forms"></a>本人の番号を手元の様式に印字する — 止めるものは 2 層、 user が外せるのは片方だけ (実測)

本人が印刷して署名・郵送する様式 (銀行・税務の届出など) に、 本人の口座番号を script で印字する場面。 止めるものが 2 層あり、 外せるかどうかが違う。

| 層 | 止めるもの | 外し方 |
|---|---|---|
| Claude 自身の安全規則 | 口座・カードの番号、 政府発行の番号 (国民番号・SSN 等)、 パスワードを入力欄に入れること | 外せない。 本人が許可しても、 CLAUDE.md や settings に書いても効かない |
| `auto` mode の classifier | 番号の入った file を書き出す call (`Credential Leakage`)、 その file の中身を script で確かめる call (`PII Data Handling`) | user が mode を `auto` 以外 (`default` や編集の承認) にする = [上の (a)](#chat-consent-does-not-reach-classifier)。 Claude は mode を変えない |

- **規則の線引き**: 規則が止めるのは、 Web やアプリの入力欄に打ち込んで番号が相手に渡ること。 手元で様式を作り、 本人が見て署名し郵送するのは、 支払いの様式に振込先の口座を印字するのと同じ行為なので、 口座番号は印字してよい。 **政府発行の番号は手元の様式でも印字しない** (本人が手書き)。 印字 tool の spec は、 その番号の鍵を見たら止まる形にしておく
- **手順**: ① 口座番号を印字する段に入る前に、 user に mode の切り替えを頼む ② 生成と印刷用の raster 化を回す (Bash は 1 本ずつ承認が出る) ③ 確認は crop した画像を目で見る。 番号を文字列で照合・表示する検査は書かない (出力に番号が出るうえ、 埋め込み font が `-` を別の dash として抽出して一致しないことがある = [`office-automation.md#pdf-text-match-nfkc`](office-automation.md#pdf-text-match-nfkc) の dash 拡張) ④ spec は暗号化した場所に置き、 commit message に番号を書かない ⑤ 終わったら user に `auto` へ戻してもらう
- **classifier に止められた直後にやらないこと**: 同じ call の打ち直し、 出力先や確認の書き方を変えて試すこと、 別 session に回すこと。 変更を戻し、 user に 2 層の違いと選択肢 (手書き / mode を切り替えて印字) を出す
- **同じ種類の番号に、 様式ごとに違う線を引かない**: 「Claude はこの番号を書かない」 と決めるときは、 同じ種類の番号を扱う他の pipeline を探して揃える。 片方だけ厳しいと「あっちでは書いているのに」 と同じ問いが戻ってくる
