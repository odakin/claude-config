...
- <a id="machine-gated-pending-action"></a>**特定マシンでしかできない残作業は「ホスト ∧ 未完了」 の条件で既配線の発火面に載せる** (2026-09-12 追加) — 例: 別マシンで作って push し忘れた repo。 条件 = `[ "$(hostname -s)" = "<host>" ] && ! <完了の機械的判定>` (完了判定の例 = `git -C <dir> rev-parse --verify -q '@{u}'` = upstream が付いたか)。 (1) 対象マシンでだけ、 完了するまで毎 session 出て、 完了で自然に止まる (= 消し忘れの無い carrier) (2) 対象 dir が無い場合も**出る**向きに書く (= `git -C` の失敗を「未完了」 側に倒す。 dir 名の取り違えで黙って沈黙しない) (3) host 名は fleet heartbeat と同じ `hostname -s` で取る (4) 検証は PATH の先頭に偽の `hostname` を置いて本物の発火経路を回し、 対象外マシンで沈黙することも確かめる (5) 発火面が repo の data を読むより先に同じ session 開始の auto-pull が済むとは限らない ([`hook-authoring.md#parallel-hooks-no-ordering`](hook-authoring.md#parallel-hooks-no-ordering)) = 1 回目の session は取りこぼしうる (6) **「動くか確かめる」 TODO は、 機能側が書く証拠 file を完了判定に使う**: 例 = hook が実行時に書く `~/.claude/state/<name>.last` → 条件 `[ "$(hostname -s)" = "<host>" ] && [ ! -f "$HOME/.claude/state/<name>.last" ]`。 対象マシンで一度動いた瞬間に TODO が自然に消える (= 人が確かめに行く手順が要らない。 機能側の書き方 = [`hook-authoring.md#opt-in-side-effect-hook`](hook-authoring.md#opt-in-side-effect-hook))。 検証は `hostname` を置換した条件を一時 `$HOME` で回し、 証拠の無い間は出る・置いたら消える・対象外マシンでは出ない、 の 3 点を見る。 条件を書く field と発火面の実装は owner の個人層 (instance-down)。
...
- <a id="one-shot-settings-propagation"></a>**machine-local な settings の値を他マシンへ届けるのは、 auto-apply 層の one-shot step** (2026-09-12 追加) — `~/.claude/settings.json` は git 非同期なので、 あるマシンで owner が決めた値 (既定 model / permission mode / allow / additionalDirectories 等) は他マシンに届かない。 既配線の SessionStart bootstrap に step を足す: (1) marker file が無いマシンでだけ適用し、 marker を書く (= one-shot。 pin にしない = 後から user が変えても再上書きしない) (2) JSON 破損等で失敗したら marker を書かず、 次の session で再試行する (3) 適用した回だけ session 冒頭に出す (4) 制限を強める追加 (ask / deny) は安全側。 権限を広げる変更は owner が明示した値だけにする — auto mode では Claude による settings の直接編集は classifier に block されるので ([`claude-code-permissions.md#protected-settings-edit`](claude-code-permissions.md#protected-settings-edit))、 git で review される bootstrap が正規の経路になる (5) symlink を含む path 設定は実体 path も足す ([`#symlink-both-paths`](claude-code-permissions.md#symlink-both-paths))。 同じ auto-apply 層は「installer が setup.sh 実行時にしか走らない」 種類の修復にも使える ([`hook-authoring.md#installer-tracked-stub`](hook-authoring.md#installer-tracked-stub) の heal)。 実装は owner の個人層 (instance-down)。
  - <a id="gate-rules-reassert-every-session"></a>**安全 gate の rule (確認を課す ask / 禁止する deny) は one-shot にせず、 毎 session 冪等に揃え直す** — one-shot の marker は「後から user が変えた値を上書きしない」 ための仕組みで、 好みの値 (model / mode) には合うが、 gate には合わない。 gate の rule は他の設定 UI・installer・手編集で**黙って消えうる**うえ、 消えると確認なしで操作が通る (実測)。 必須 rule の宣言を git に置き、 session 開始の auto-apply 層が毎回「足りなければ足す」 (変更が無ければ無音、 JSON 破損は書かずに次 session 再試行)。 ⚠️ **rule を絞ったときは「旧 rule → 置き換え先」 の対応も宣言する**: 新しい狭い rule を足すだけだと広い旧 rule が残り、 誤爆 (ask) や広い許可 (allow) が続く。 かといって「宣言に無い rule を消す」 にすると、 他の層や user が足した rule まで消える。 置き換え表で名指しした旧 rule だけを同じ位置で置き換える。 engine = [`scripts/sync-permission-rules.py`](../scripts/sync-permission-rules.py) (`--spec` の JSON に `required` / `superseded` を書く。 監査 = exit 1 で列挙 / `--apply` = 冪等に揃える。 spec 中の `${HOME}` は各マシンの home に展開されるので、 絶対 path を含む rule も 1 つの宣言で配れる)。 ⚠️ **gate の kind を変える判断 (例: 触れなかった dir を、 確認つきで開ける `ask` にする) も宣言に載せる** — 変えたマシンだけ通って他は通らない状態を人の手作業で埋めさせない。 旧 kind に残る rule の形がマシンごとに違って列挙できないときだけ `retired_re` (= その kind に在ってはいけない rule の正規表現) を使い、 **取り除いた rule は 1 行ずつ surface に出す** (= gate が黙って緩んだ状態を作らない)。 それ以外は名指しの `superseded` が既定。 ⚠️ auto mode では、 ask rule を**絞る** Claude の直接編集も block される側に入る (= 確認が減る = 権限の拡大として扱われる、 実測)。 この経路でも「git に載せた宣言を bootstrap が適用」 が正規の道。
  - <a id="version-gated-propagation"></a>**届ける値が道具の版を前提にするとき (新しい model の名前など) は、 その値を実際に使う binary の版を確かめてから書く** — 同じマシンでも経路ごとに binary が違う (アプリの内蔵・端末の CLI・常駐 server・定期実行の PATH の CLI)。 版の足りない経路に書くと、 その経路だけが毎回拒否で失敗する (実測: 古い CLI は新しい model を 400 で拒否する)。 (1) 経路ごとに実行される binary の版を読み、 足りた経路だけ書いて marker を置く (2) 足りない経路では marker を置かずに保留を 1 行出す = 版が上がった次の session で自動で入る (3) 定期実行の定義に焼き込んだ値 (plist の起動行の model など) は ensure が触らない手の指定なので ([#ensure-reconciles-content](#ensure-reconciles-content) の 1)、 一度きりの段でその部分だけを差し替える (入れ直すと install 時の他の指定が消える)。 常駐 server は update しても古い版のまま走り続ける = [`remote-control-server.md#update-needs-restart`](remote-control-server.md#update-needs-restart)
- <a id="contradicting-observations-are-machine-split"></a>**前の記録と食い違う実測が出たら、 まず machine 差を疑う** (2026-09-19 追加) — machine-local な状態 (settings の rule・machine-local な一覧 file・install 済みの物) を、 machine 軸を持たない doc に「実態はこう」 と書くと、 別マシンの session が同じ行を逆向きに書き直す。 **往復は「どちらの観測が嘘か」 ではなく「doc に machine 軸が無い」 の症状**で、 実測 1 例では同じ日のうちに同じ行が deny ⇄ ask に書き換わった (どちらの観測もそのマシンでは正しかった)。 直し方:
  1. **発生元マシンを判定する**: その session の記録が手元にあるかで分かる ([`scripts/search-agent-transcripts.py`](../scripts/search-agent-transcripts.py) `--session <id>` が空 = 別マシン発)。 file の mtime と「いつ書き換わったはずか」 の突き合わせも効く
  2. **実態は machine 別の比較行を持つ doc に置き**、 入口の doc (= 毎 session 読まれる方) は pointer だけにする ([`../docs/personal-layer.md`](../docs/personal-layer.md) の machine 別分岐)
  3. **記述を合わせるのでなく、 状態を収束させる**: 宣言を git に置いて全マシンで揃える ([#gate-rules-reassert-every-session](#gate-rules-reassert-every-session))。 そうすると doc は「宣言がこう」 の 1 本になり、 往復の余地が消える
  4. 往復した topic は**重複検出の登録**に足す (= 同じ事実が入口 doc に再び書き写されたら機械が言う)
- <a id="ensure-reconciles-content"></a>**自己修復の ensure は「在るか」 だけでなく「中身が宣言どおりか」 も見る** — 未登録の物だけを入れる ensure は、 宣言の変更 (時刻・条件) も engine 側の変換の変更も、 既に入っているマシンには届けない (= 各マシンで手の入れ直しが要り、 忘れたマシンだけ旧い挙動のまま走る。 実測)。 直し方の要点:
  1. **書き換えるのは宣言から一意に決まる部分だけ**。 丸ごと作り直すと、 install 時に手で渡した指定 (model・account・gate 等) が黙って消える。 手の指定が乗る側の変更は ensure では扱わず、 明示の入れ直しか、 その部分だけを差し替える一度きりの段 ([#version-gated-propagation](#version-gated-propagation) の 3) に残し、 その境界を doc に書く
  2. **照合済みの印** (宣言の値 + 変換の版) を置き、 印が一致すれば実物を読まない (= 毎 session 呼ばれても定常時は軽い)。 印は cache で正本は実物。 変換の意味を変えたら版を上げて全件を 1 回照合し直させる
...
7. **監視 layer の分界 — heartbeat は「agent の生死」 だけを見る**: launchd job が「生きて」 いて**毎回失敗している** (= `launchctl list` の LastExitStatus ≠ 0) 状態は heartbeat の射程外 — job は定刻に走り、 beat も打たれ続けるからである。 実 incident (2026-07-29): 日次 routine 5 本が context 超過で 4-5 日連続 exit 1 だったが、 exit status を見る機械が無く沈黙した ([scheduled-tasks.md #headless-context-budget](scheduled-tasks.md#headless-context-budget))。 監視は 3 層で相補する: ① agent/machine の生死 = heartbeat (本 pattern) ② **job の exit status** = LastExitStatus watcher (機械実装は各層) ③ 成果物の鮮度 = 各 routine 固有の heartbeat file (= exit 0 で内容が壊れる silent 論理故障を拾う)。 1 層でも欠けるとその class の失敗が黙る。 ⚠️ ② の watcher が**その機械の中**でしか出ないと、 誰も session を開かない常駐機の失敗は見えない = ②を他の機械へ運ぶ opt-in が writer の **job health** (`--job-label-prefix` + `--job-python-modules`): 各 job の最後の終了コードに加え、 job の PATH で解決した python3 が起動できるか・依存を import できるかと、 command / wrapper が `python3` を PATH で呼ぶかを記録し、 reader が他の機械の分を出す。 exit 0 のまま何もしない job (依存の import で fail-open に終わる) は ② でも ③ の無い job では見えないので、 interpreter の健康を直接見る ([`shell-env.md#job-python-by-capability`](shell-env.md#job-python-by-capability))。 ⚠️ ② の終了コードは plist の読み込み直しで 0 に戻り、 原因も言わない = ② は直近 run の log 末尾も読む (job health は `log_failure` = Claude の認証切れ / Prompt is too long を運び、 reader は exit 0 に隠れた失敗と 認証切れの config dir を出す。 判定は [`scripts/lib/launchd_job_log.py`](../scripts/lib/launchd_job_log.py) の 1 か所で そのマシンの horizon と共有、 [`scheduled-tasks.md#reload-resets-exit-status`](scheduled-tasks.md#reload-resets-exit-status) / [`#headless-auth-expiry`](scheduled-tasks.md#headless-auth-expiry))
8. **欄の読み方 — `Connected` と `default` は認証の証拠にならない** (実測): RC サーバーの `last_status = Connected` は、 更新トークンが切れた後も残る (待ち受けは更新を使わない経路で動く)。 `config_dirs.default` は desktop app を最後に起動した時の desktop の account で、 ログインの記録ではない (desktop app の起動がこの欄を書き換える)。 設定フォルダの認証は、 見張り ([`scripts/check-desktop-logout-auth.py`](../scripts/check-desktop-logout-auth.py)) の問い合わせの結果を writer が `config_dir_auth` に載せ、 reader が他のマシンの分を 🔴 で出す (writer は記録と keychain の更新時刻を読むだけで、 1 の原則を保つ)。
