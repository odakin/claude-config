...
- [`scripts/pre-commit-google-url-warn.py`](../scripts/pre-commit-google-url-warn.py): lifecycle hook の出力が見えない frontend でも生きる Git-native fallback。staged 追加行だけを同じ二軸で warn し、低 stakes のため commit は止めない。
