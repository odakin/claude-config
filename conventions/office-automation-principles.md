...
## <a id="forms-are-not-data"></a>0. 問題の本質: 様式ファイルは「データ」ではない
...
## <a id="tool-selection-ladder"></a>1. 道具を選ぶ前に問う 2 つの質問
...
| **docx → PDF** (内容確認・正式書類提出いずれも) | **Word AppleScript 駆動が default** ([`docx-to-pdf-pages`](office-automation.md#docx-to-pdf-pages) / [`docx-pdf-stale-cache`](office-automation.md#docx-pdf-stale-cache))。 既存 wrapper = `scripts/docx-to-pdf.sh` (引数なし = Word、 `--pages` で明示 Pages)。 Word / Excel / PowerPoint 経路は**事前 grant 済み staging dir 経由が default** ([`office-pregranted-staging-dir`](office-automation.md#office-pregranted-staging-dir) = Office の sandbox dialog を案件 dir ごとに踏まない) | docx は「Word 体裁が契約」 の正式書類が大半。 Pages は re-typeset で見出し/表が重なる artifact + reviewer は Word 体裁を見る。 Word automation の cold-start trap は [`docx-pdf-stale-cache`](office-automation.md#docx-pdf-stale-cache) で対処済 (= fallback 1 = user の Word 書き出し) |
...
2. **Office に触らせる file は `scripts/lib/office-staging.sh` の staging 経由にする** (= [`office-pregranted-staging-dir`](office-automation.md#office-pregranted-staging-dir)、 Office の folder-grant dialog を構造的に回避 + 原本を export 時の再保存から守る)。 その上で **`/tmp` 配下を docx の作業場所にしない** = script が docx を生成 / 受領 / 中間保存する場所は project 配下 (= cwd か `~/<repo>/...`) を default に。 `/tmp` を input/output に取る引数は [`docx-tmp-sandbox-deny`](office-automation.md#docx-tmp-sandbox-deny) と同じ warn (= block でなく user 判断) を出す。
...
## <a id="three-layer-verification"></a>2. 検証の 3 層モデル (= どれも他の代わりにならない)
...
## <a id="nfkc-string-match-trap"></a>3. 文字列照合の罠 (= 「無い」 という判定を信用する前に)
...
## <a id="derive-coordinates"></a>4. 座標・cell 番地は「導出」する (= hardcode しない)
...
## <a id="provenance-skepticism"></a>4b. 「テンプレ」「前例」 の肩書きを信用しない (= 素性は実 file に聞く)
...
## <a id="human-systems-principles"></a>5. 人間系の原則 (= 様式仕事は対人サービスである)
...
## <a id="reviewer-eye-completion"></a>5b. 記入後は「審査員の目」 で閉じる (= reviewer's-eye 完成度監査)

記入の検証を「**自分が入れた値が着地したか**」 で終えると、 **記入すべきだった所の取りこぼし・ラベル欄への上書き・配置崩れ**が素通りする。 PDF を render して目視しても、 人は**自分が触った所**しか見ない。 検証の単位を「自分の記入」 から「**審査員が見たときに 空欄・誤記入・崩れが無いか**」 に切り替える。

1. **機械監査を先に通す**: 様式記入の最後に **blank vs 記入版の diff** を必ず実行 — xlsx は [`diff-form-xlsx-detection`](office-automation.md#diff-form-xlsx-detection)、 **docx は [`diff-form-docx-detection`](office-automation.md#diff-form-docx-detection)** (`diff-form-docx.py`)。 ラベル上書き・見出し消失 (HARD) / 空の箇条書き・全空の labeled 列 (surface) を機械が拾う。
2. **その上で reviewer's-eye 視覚 pass**: 全ページを「初見の審査員」 として通し、 **labeled な欄・列・箱が 記入済 or 意図的空欄か**を 1 つずつ確認する (= 「自分が記入した所の確認」 ではなく「全 labeled 域の点呼」)。 意図的空欄 (印・提出日・財団記入欄・該当なし欄) は理由を即答できること。
3. **機械に乗らない 1 点 = 中身の質**: 「箱が半分」「散文が薄い」 は機械では判定できない (= 構造でなく内容)。 申請の核 (≪概要≫ 等) は**実質で埋め、 重要な箱を半分で残さない**。 これは人の目が最後の砦。
...
## <a id="extending-the-catalog"></a>6. 新しい罠に出会ったときの手順 (= 本体系の拡張方法)
...
## <a id="one-liner-summary"></a>7. 1 行 summary (= 迷ったらこれだけ)
