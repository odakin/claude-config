...
reflex: 「この run はどの経路?」 には (a) 内容指紋を 1 つ挙げたか、 「こっちでは起きない」 には (b) その経路がそこで走っている証拠を見たか、 「check が negative」 には (c) check 自身の脚を疑ったか、 source から挙動を主張する時は (d) その source が実行実体である binding を見たか。
