...
1. **着手前に live で最新 run を再照会する** — 依頼文・SESSION・spawn spec の「CI が N 回連続で落ちている」 は起票時点の snapshot。 `gh run list --limit 5` を newest first で 1 回見る。 実例 (2026-09-11): 「直近 100 run すべて failure」 という spec を受けた時点で、 別 session の修正 commit が約 2 時間前に push され、 後続 4 run が green だった。 spec どおりに「直す commit」 を作っていれば重複修正になっていた。 受け手側の一般則 = [`multi-session-coordination.md#receiver-premise-snapshot`](multi-session-coordination.md#receiver-premise-snapshot)。
...
origin: 2026-09-12。前日の事故を捕まえる検査を書いた当日に、当の案件が**再提出された**ことで
