...
| 3 | 濃部 R 一致 → サイズ裁定のみ (初周の 31pt 推定は測定過大、 真値 ≈24pt、 持ち主許容の確定値 28pt) ⚠️ **この 28pt は当日の中間値** — 後日 (2026-07-27 円ターゲット較正) に engine が PNG の content fill を自動補正する仕様と判明し、 **指定 = 紙上のインク径 27pt** が確定値になった (= 迷走中の 28/31/33pt は全て obsolete。 正本 = 個人層の印鑑 § / 一般則 = [#seal-size-determination](#seal-size-determination)) | 変換なし・overlay サイズ確定 |
...
- **色補正の overshoot に注意**: 「もう少し明るく」 に対して大きく動かすと 1 周で行き過ぎる (実測: G×1.22/B×1.30 は「だいぶ違う」 で却下、 正解は中間ですらなく元の値に近かった)。 判定が写真ごとに揺れるときは**紙白正規化** (= 同 crop 内の最明部を 255 に正規化してから濃部比較) で照明差を消してから方向を決める。

### <a id="seal-size-determination"></a>サイズ確定の正しい手順 (= 迷走 5 周分の RCA から)

印影のサイズ校正には**隠れた系統誤差**が 2 つあり、 これを先に潰さないと loop が収束しない (= 実測 5 周迷走した):

1. 🔥 **PNG の content fill** (= 最重要 RCA): 印影 PNG は canvas に余白を持つ (例: 600px canvas に印影径 537px = fill 0.895)。 `insert_image` は **canvas を rect に fit** するので、 **紙上の印影は指定サイズ × fill にしか出ない** (= 11% の系統的過小)。 全てのサイズ評価がこの係数分ずれ続け、 「実物に合わせたはずが小さい」 を何周も繰り返した。 → engine ([`overlay-seal-pdf.py`](../scripts/overlay-seal-pdf.py)) が PNG の不透明 bbox から fill を自動測定し rect を 1/fill 倍して補正 = **指定 pt = 紙上のインク径**を保証。 自前実装するときも必ずこの補正を入れる。
2. **プリンタの縮小疑い**: 「サイズが合わない」 ときに fit-to-page 縮小を疑ったら、 **100mm 線 + 既知 pt の円**を印字した較正ページで検証する。 定規が無くても **A4 を短辺方向に半分に折れば 105mm 基準**が作れる (100mm 線との差 5mm = 100% / 目視で分かる差 = 縮小)。 実測では 100% だった (= 陰性確認も価値がある: 容疑者を 1 つずつ潰す)。

**確定の 4 段**:
1. **第一推定 = 印面の規格径**。 シャチハタ系は規格が公開されている: ネーム6 = 6mm / ブラック8 = 8mm / **ネーム9 = 9.5mm (定番)** / ブラック11 = 11.5mm。 持ち主に型番か軸の刻印を見てもらうのが最速。
2. **円ターゲット較正** (= 定規不要の決定打): 既知 pt の円を 3 サイズ印字し、 **実物を円の中に押してもらう**。 包含関係が一目で分かり、 円 (ベクタ描画 = 正確) を基準に押印の px→pt 換算もできる。 実測でネーム9 の紙上押印 = 26-27pt ≈ 規格 9.5mm と一致。
3. **engine の fill 補正で「指定 = インク径」を保証**した上で、 規格径を指定。
4. **最終ゲート = 実際の書類上での実物押印との並置目視** (= 持ち主の判断。 ここまでの 3 段が正しければ 1 発で通る)。

⚠️ **「押し圧で径が変わる」 説に逃げない**: 押し圧による滲み拡大は高々 数%。 測定間で 15% 以上食い違ったら、 押し方でなく**測定系か描画系の系統誤差** (上記 1-2、 または閾値・手法の測定間差) を疑う。 実測でも「押し圧説」 は誤帰属で、 真因は content fill だった。
