...
- **提案を PDF に印字・着色する指示とは解釈しない。** `\red{}`・`\CL{}` 等による review 注記の挿入や本文の着色は、ユーザーがその出力を明示的に指定した場合だけ行う。既存の注記・色は今回の変更対象でなければ保つ。
