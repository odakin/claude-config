...
## <a id="bare-parenthetical-crossrefs"></a>裸の括弧参照の検出と保持検査
...
修正は自動置換しない。参照先を読んで定義・導出・検証・設定の関係を判定し、本文の動詞や前置詞に結び付ける。候補ゼロや保持検査の成功は文意の正しさを保証しない。文意の確認と再ビルド後の目視確認を別に行う。
