...
- **PDF 読み取り (= 受領 PDF から値を取る、 読み側も fill 側と同格の工程)**: [`pdf-table-layout-aware-reading`](office-automation.md#pdf-table-layout-aware-reading) (= **表・多段組は plain `get_text()` 禁止** — words+bbox y-band → dict → find_tables → clip の抽出 ladder、 行帰属は y-band 一致を確認してから主張、 「読めない」 結論は ladder 全段の後)
