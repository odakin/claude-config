...
| [`check-form-static-text.py`](../scripts/check-form-static-text.py) | **雛形が元から紙に出すもの** (図形の標題・区分の枠・様式番号・㊞) が出力 PDF に在るか = openpyxl で保存した workbook / temp から刷った紙の見出し消失・枠で末尾 1 字が切れた様式番号を拾う (2026-09-24 追記。 他の検査は書いたものしか見ない) → [`#drawings-lost-in-temp-pdf`](office-automation.md#drawings-lost-in-temp-pdf) |
| 同 `--filled` / `--blank` / docx 雛形 (2026-09-25) | 書き換えていない cell の見出しが出力に在るか (`--filled 案件.xlsx`) / 素刷り (雛形を道具を通さず app で刷った PDF) と画像・線の数を比べる (`--blank`、 Excel は checkbox の箱を画像として描く) / docx 雛形の textbox・header・footer の字 → 不変条件と層 = [`form-case-pipeline.md#fidelity`](form-case-pipeline.md#fidelity) |
