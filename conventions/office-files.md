...
- **紙印刷**: [`pdf-prefill-direct`](office-automation.md#pdf-prefill-direct) (= 紙だけ要る時は雛形 PDF + fitz 印字、 汎用 engine = `pdf_form_fill.py`) / [`print-raster-pdf`](office-automation.md#print-raster-pdf) (= 600dpi ラスタ化で printer font 化け回避) / **刷る直前 = [`print-preflight`](office-automation.md#print-preflight)** (= `scripts/pdf-print-preflight.py` で頁数・font を機械確認 → RGB raster → crop 目視 → remote user には全頁 PNG → 1 枚ずつ。 2026-08-21 に 4 回刷り直した RCA)
