...
<a id="edit-empty-new-eats-newline"></a>**編集 tool で置換後を空にすると、 直後の改行も消えることがある** (実測): Edit の `new_string` を空にし、 `old_string` が行の途中から行末までだったとき、 次の行が前の行につながった。 行の一部を消すときは、 前後の文字を `old_string` に含めて空でない `new_string` で置き換え、 置換後の file を期待した全文と `cmp` で照合する。 規則保護の gate は置換を自前で再現して判定するので、 この差は編集の時点では止まらない (commit 時の gate は staged の全文を見るので、 そこで止まるはず = 未確認)。
