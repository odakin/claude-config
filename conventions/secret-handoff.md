...
## <a id="do-not-store-inside-the-lock"></a>secret を「それが開けるもの」 の中だけに置かない

復旧のための secret (ディスク暗号の復旧キー / 2 要素認証の backup code / 鍵を復号するパスフレーズ) は、
**それが要る場面 = その資源に入れない場面**。 ∴ その資源の中だけに置いた copy は、 要るときに必ず読めない。
平時は問題なく見えるので、 気づくのは要った瞬間 = もう手遅れの瞬間になる。

置き場の要件:

- **別の資格情報で、 別のデバイスから開ける**こと (= 同じ鍵・同じ端末に依存しない)
- 同時に失う相関のある 2 か所は、 独立な経路と数えない (= 同じ端末の 2 つの dir、 同じ account 配下の 2 つの sink、
  同じ建物の 2 台)

検査は 1 つの問いを 1 回たどるだけ:

> **この secret が要る状況を仮定したとき、 その状況のまま置き場に到達できるか**

到達経路の途中に「その secret が守っている資源」 が出てきたら循環している。 出てこなければ非循環だが、
**経路に出てくる資格情報 (別 service の login 等) を失う場合も 1 度想定する** — そこが単一障害点になる。

例 (形だけ): ディスク暗号の復旧キーを、 そのディスクの中の暗号化台帳にだけ置く = 循環。
別マシンから clone できる repo に暗号化して置き、 その復号鍵を別 sink に置く = 非循環
(前提 = **その 2 つに別デバイスからログインできる**。 この前提自体を台帳に書いておく)。

⚠️ 非循環にしても「全端末を同時に失う」 までは守らない。 そこまで要るなら **oﬄine の物理 copy を 1 部**。
要否は資源の価値で決める判断であって、 既定で足す物ではない。
