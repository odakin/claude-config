...
| 形 | 実例 (2026-09-13) | 処方 |
|---|---|---|
| **指示語・対語の先行詞** | `the former`、`for either field`、`these combinations`、`Each line`、`between the two regularizations` | 名詞か式番号で言い直す (`Only $K_{[\lambda\mu\nu]}$ enters …`、`the left-hand sides of Eqs. (83)–(86)`、`between minimal subtraction and the cutoff`)。対語は 2 項が同じ文に在っても、間に修飾が挟まれば崩れる |
| **前置の限定句が等位の後半まで掛かる** | `In dimensional regularization the divergence appears as the pole …, and the bare action is specified at Λ` (後半まで dim reg の話に読める) | 限定句を掛けたい文だけに置き、文を切る。後半は別の文で主語から始める |
| **射程語 (反対称化・trace・部分) の範囲** | `the totally antisymmetric part` (微分添字まで反対称化するかで別の項になる)、`the vector trace` (どの 2 添字の縮約か) | 記号で書く: `$K_{[\lambda\mu\nu]}$`、`the trace vector $K^{\lambda}{}_{\mu\lambda}$`。語は定義の場で 1 回だけ |
| **display の行位置** | `the second line of Eq. (43)` (式の改行が変われば黙って別の項を指す) | 項の名前か項の順番で呼ぶ (`the two contorsion terms of Eq. (43)`)。 [`scripts/check-paper-prose.py`](../scripts/check-paper-prose.py) の P が `… line of` / `in the … line` を列挙する |
| **排他・必然の modal** | `must be generated by its quantum fluctuations`、`only through quantum effects` (「ω を積分すれば e の運動項が出る」 という既知の反論を招く) | 論文が既に引いている線引きの語 (`of its own`、`at tree level`) を残して、事実を平叙で書く。排他を言う必要があるのは序論の定義の場だけ |
| **結論が依存する選択を落とす** | `here they are outputs of a single spinor loop` (非最小結合なら自由パラメータ) | 同じ文に選択を付ける (`here, under minimal coupling, …`)。同じ主張の他の出現 (abstract・結語) を grep して射程を揃える |
| **標準的な操作を必然と読ませる** | `Once ω is eliminated by its equation of motion, …` (本稿の立場では ω は補助場でなくなる) | 条件文にし (`In the standard treatment, where ω is auxiliary and eliminated, …`)、本稿の変数での対応物を言う |
...
**なぜ別節か**: relocation sweep は「前は正しかった束縛が壊れた」 を探すので、新文には発火しない。こちらは書いた瞬間から一意でない形で、書き手は自分の意図で補って読むため自分では気づかない (同日、 1 文を 3 回書き直してまだ「with the vector trace が何に掛かってるか分かりにくい」 と戻った例がある)。
...
| 計算済み・本稿には結果の文だけ | 「計算して確かめた」 とだけ書き、**節や式を典拠として引かない** (2026-09-13: 「§3.2 の混合振幅で確認」 「reported in §3.2」 は、§3.2 に振幅も確認の過程も無いので読者が探して見つけられない。 別の段落でその事実に触れる必要が無ければ、触れない) |
...
極を指す語は機構語 (`negative-norm pole`) に。 文献の使用頻度も通し名の選択根拠になる (全文検索で "Ostrogradsky ghost" 346 : "Stelle ghost" 3)。
