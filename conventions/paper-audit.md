...
**実例の形 (2026-09、盲検模擬審査で観測)**: 派生文書が中心に据えた「自著の解析と先行研究の予言は
食い違う。これを決着させるのが第一の挑戦」に対し、引用された自著の**脚注**が「本解析は 2 つの不確定性が
同程度という仮定を置いており、当の系ではその階層が破れる。この場合、結論は必ずしも従わない」と留保し、
結語も同じ系を「今後の課題」に送っていた。= **派生文書が主張した「矛盾」は原論文の適用範囲外への外挿**で、
専門が近い読み手が [X] を開けば 1 分で崩れる。**留保は脚注と結語にあり、本文の grep では出てこない**のが
この型の要点。
