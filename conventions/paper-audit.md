...
**実例 (2026-09、挑戦的研究の調書の盲検模擬審査)**: 調書が中心に据えた「A の解析と B の先行研究の予言は
食い違う。これを決着させるのが第一の挑戦」に対し、引用された自著の**脚注**が「本解析はエネルギーと運動量の
不確定性が同程度という仮定を置いており、B が扱う系ではこの階層が破れる。この場合、小さいエネルギー不確定性
は必ずしも振動の消失を意味しない」と明記していた。加えて本文の結語は同じ系を「今後の課題」に送っていた。
= **調書が主張した「矛盾」は原論文の適用範囲外への外挿**で、専門が近い審査委員が [X] を開けば 1 分で崩れる。
