...
機関事務のチェック → 差戻しは**正常フロー**（初回でノーチェック通過する方が例外。実応募 2 件で v1→v4+ の複数回提出・差戻しを観測）。パターン: **修正依頼メール + 赤字書き込み PDF 添付**（添付忘れ・コード誤記の訂正再送で同 thread に 2-3 通来ることがある → **最新の訂正版を正とする**）、**再提出期限は短い**（翌日正午等）。⚠️ **指摘は電話でも来る**（実応募で 3 件中 2 件が電話。メールに残らないので、聞いた内容を**直ちに記録してから着手**し、対応完了報告メールに指摘内容を復唱して書面化する）。e-Rad の「修正依頼通知」自動メールは**コメント欄が空**のことが多く、詳細は事務からのメール/電話が別便で来る（1.5〜2 時間差の実例）— 通知だけで理由を推測せず詳細を待つか事務に確認。
