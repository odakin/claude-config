...
- **environment の identity は client 側 `projects/<dir>/bridge-pointer.json` が保持** (2026-07-04 実測): server 再起動は同 env に再接続する (= "Environment preserved")。 **fresh 登録を強制**するには server bootout → bridge-pointer.json を退避 → bootstrap (= 新 env id が発行される)。 ⚠️ 旧 env は picker に **dead 残骸として残る** (= 死んだ session 残骸と同様、 UI から削除するまで並ぶ)。
