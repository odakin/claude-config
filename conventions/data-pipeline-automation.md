...
| 読み手の 1 つが**共有の正規化関数を使わず独自に読む** | 他の読み手が認識する表記 (短縮形等) を、 その読み手だけが未認識として出し続ける (実測、 [`email-surface-pattern.md#recorded-id-notation`](email-surface-pattern.md#recorded-id-notation)) |
...
- **正規表現の捕捉群を共有 API の外へ漏らさない**。 Python の `findall()` は捕捉群が 1 個なら文字列、複数なら tuple を返す。表記を増やして捕捉群を足すだけで、直接 `findall()` を呼ぶ別の読み手の key 型が変わる。正規化関数は最終的な文字列 key を返し、正規表現を変えたら全消費者の selftest を回す。
...
## <a id="silent-sentinel-serialization"></a>12. 「無い」 を表す値は、 経路のどこかで文字列に化けて invariant を素通りする

集計 pipeline で「未実施 / 該当なし」 を `null` (Python の `None`、 YAML の `null`) で
表すのは正しい。 危険なのは **その値が層をまたぐとき**で、 sentinel は静かに型を変える:

- `f"{k}: {v}"` に `None` を入れると **文字列 `"None"`** になる。 YAML で読み戻すと
  `null` ではなく 4 文字の文字列 ∴ `x is None` が false、 `--check` は「不一致」 を
  大量に出すが、 その不一致は**データの誤りでなく直列化の誤り**。
- 表示層で `f"{v:>6}"` に `None` を渡すと **TypeError で落ちる**。 落ちた場所が
  「集計 → 表示 → 書き込み」 の途中だと、 **書き込みに到達しないまま「実行した」 と
  読み違える** (= 標準出力に途中までの表が出ているので成功に見える)。

### 規律

1. **sentinel の直列化を明示する**。 `None` を文字列化する経路を作らない
   (= `"null" if v is None else v` を書き出し関数の 1 箇所に置く)。
2. **selftest に sentinel の往復を入れる**。 「書いて読み戻して `is None`」 まで見る。
   値の一致だけを見る test は文字列 `"None"` を通してしまう。
3. **表示は sentinel 安全に**。 集計層が `None` を返しうるなら、 表示側は必ず
   `"—"` 等へ落とす。 表示で落ちると**書き込み step に到達しない**。
...
## <a id="derivation-covers-non-participants"></a>13. 導出は「対象になった集合」 でなく「対象になりうる集合」 を走査する

複数の入力を合算する導出 (= 成績・請求・在庫の集計) で、 **主たる入力に現れる ID
だけを回す**と、 副次入力にしか現れない対象が丸ごと落ちる。 落ちた対象は
「値 0」 として表示されるので、 **欠測と 0 が区別できない**まま下流に流れる。

- ✗ `for x in 主入力:` … 主入力に無い対象は存在しないことになる
- ✓ `for x in (主入力 ∪ 副入力1 ∪ 副入力2 ∪ 母集団):` … 母集団を持っているなら必ず含める

⚠️ **「主たる入力」 を無意識に母集団だと思い込む**のがこの失敗の本体。 主入力は
たいてい「参加した人」 であって「対象者」 ではない。

初出: 成績集計の pipeline。 手書きの提出物の転記に現れる学生だけを走査していたため、
**試験を欠席した学生が実際に取っていたレポート点・実験点が消えた** (= 合計 0 と
表示されるが、 実際には満点を取っている者が複数いた)。 母集団 (履修者名簿) を
走査集合に加えて解消。

## <a id="display-bound-field-purity"></a>14. 下流出力に直結する field に、 人間向けの注記を同居させない

DB / yaml の 1 field が **生成物にそのまま印字される** (= 掲示物・公開ページ・定型メール・様式) とき、
その field は「人間が読むメモ欄」 ではなく **出力そのもの**である。 ところが field は編集する人間から
見ると常にメモ欄に見えるので、 運用の都合 (= 期間・状態・確認事項) が自然に混入する:

- 所属を持つ人物 DB の `affiliation` に滞在期間の注記を書く → **掲示ポスターにその期間が印字される**
- 様式の label セルに説明文を書く → 印刷物に説明文が出る (= [`office-automation.md`](office-automation.md) の label 上書き系と同根)

**規律**:

1. **field の purity を宣言する**: 出力に流れる field は「対外表示可能な値のみ」 と comment で明示し、
   運用注記は `notes` 等の **出力に流れない field** に置く (= 分けるのが唯一の確実な防御。
   「短く書けば大丈夫」 は判断であって invariant ではない)。
2. **新しい consumer を足す瞬間に既存 field を疑う**: field は元々「人が読む台帳」 として始まり、
   後から生成物の入力に昇格することが多い。 **昇格の瞬間が混入の検査点** —
   既存全 record をその field について一度目視する (= 昇格前に書かれた注記は例外なく混入候補)。
3. **出力の目視で拾えたものは field 側で直す**: 生成物を見て気づいた不要語を生成物側で削ると、
   同じ field を読む他の consumer (= 別の mail template / 別の頁) では直らない。
   直すのは常に SoT field 側。
