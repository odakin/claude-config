...
│   ├── claude-app-cwd-pin.md # Claude.app (Cowork desktop) の新セッション folder picker 起点を `<base>` に固定する launchd エージェント (= NSNavLastRootDirectory を 2 秒間隔で書き戻し、 picker の drift 防止。 setup.sh Step 2b2 が macOS で default-ON install、 opt-out marker `~/.claude/pin-claude-cwd.off` / `CLAUDE_PIN_CWD=0`、 launchd ThrottleInterval=10s 罠の対処込み)
...
├── scripts/
│   ├── pin-claude-cwd.sh               # Claude.app folder picker 起点固定 (= `defaults write NSNavLastRootDirectory $1`、 setup.sh Step 2b2 の launchd から 2 秒間隔で呼ばれる、 macOS 限定、 conventions/claude-app-cwd-pin.md)
...
4. *(macOS のみ)* PATH 消失防止（`.zprofile` の重複 `brew shellenv` 修正 + スナップショット自動パッチ用 launchd エージェント）+ **Claude.app の新セッション folder picker を `<base>` に固定する launchd エージェント**（default-ON、 opt-out 可 = `touch ~/.claude/pin-claude-cwd.off` or `CLAUDE_PIN_CWD=0`。 詳細・除去手順は `conventions/claude-app-cwd-pin.md`）
