...
├── scripts/              # 運用 script 群 (93 file + lib/ 10 helper。 全列挙 + 説明 = scripts/README.md 〔生成物〕、 説明の源 = 各 file header 1 行目)
...
8b. 全リポに prepare-commit-msg hook をインストール（commit message の trailer に `Claude-Session: <CLAUDE_CODE_SESSION_ID>` を 1 行足す = 並列 session の commit を事後に区別する forensics。 ⚠️ Step 8 と違い `.claude/public-repo.marker` を要求しない = **全 repo 対象** — trailer は session id しか 書かず public / private で挙動が変わらないため出し分けが不要、 かつ private repo〔= 並列 session が同じ tree を 触る作業場〕でこそ効く。 env が無ければ no-op〔= trailer の無い commit は AI 経由でないと読める〕、 既存 trailer が あれば追記しない〔= rebase 混入の回避〕、 fail-open〔= commit を止めない〕。 repo 単位の opt-out = `git config claude.sessionTrailer false`。 設計判断 = [`DESIGN.md#session-provenance-trailer-design`](DESIGN.md#session-provenance-trailer-design) / 規約 = [`conventions/multi-session-coordination.md#session-provenance-trailer`](conventions/multi-session-coordination.md#session-provenance-trailer)）
