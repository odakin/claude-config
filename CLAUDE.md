...
├── scripts/              # 運用 script 群 (156 file + lib/ 15 helper。 全列挙 + 説明 = scripts/README.md 〔生成物〕、 説明の源 = 各 file header 1 行目)
...
- **機械**: 公開 repo の pre-commit と commit-msg の **Tier E** = [`scripts/check-activity-facts.py`](scripts/check-activity-facts.py) (個人層の固有語 = BLOCK / 出来事の語と日付・件数が同じ行 = 警告のみ / `--scan-tree` = 承認済み一覧つきの棚卸し)。 日付も件数も無い言い換えは圏外なので、 hoist する turn で上の問いを自分で当てる
