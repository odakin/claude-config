...
If a task starts in a parent workspace and only later selects a nested repository, the agent must read that repository's root `AGENTS.md` before acting; for sustained work, the task should start with that repository as its project root so native discovery covers it. A machine-local fallback filename is optional convenience, not cross-machine or collaborator evidence. `scripts/setup-codex.sh` must not create or edit layer-2 project files: templates create the entry point during project setup, and `scripts/audit-codex-integration.sh --repo <path>` reports a missing, untracked, symlinked, or incomplete root entry point. A base directory that merely holds repositories is not itself a repository, so no repository entry point covers a task started there. `setup.sh` places a thin `AGENTS.md` at that base directory from `templates/root-AGENTS.md.default` (never a symlink to a repository's entry point, whose relative pointers would not resolve there); it routes to the base `CLAUDE.md` and tells the agent to read the entry point of whichever repository the work is in. `scripts/audit-codex-integration.sh` reports it missing. At write time, the shared pre-commit runners warn (without blocking) in any repository whose index has a root `CLAUDE.md` but no `AGENTS.md` (`scripts/lib/agents-entrypoint-warn.sh`), so a repository created without the entry point is noticed by whoever commits next instead of waiting for someone to remember an audit.
