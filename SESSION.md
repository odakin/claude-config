...
Codex の変更 task 完了 gate は、共通の意味契約を [`CONVENTIONS.md#completion-git-gate`](CONVENTIONS.md#completion-git-gate)、Codex 固有の発火・検査・限界を [`codex/PARITY.md#completion-git-gate-hook`](codex/PARITY.md#completion-git-gate-hook) が所有する。現在の検収はその 2 正本と aggregate checks / layer-4 audit から再開し、この SESSION に規則や hook 詳細を複製しない。
