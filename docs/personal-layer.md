...
The executable form of this split—generic engine above, owner config and stable-path shim below—is specified in [`script-layer-placement.md#engine-instance-seam`](../conventions/script-layer-placement.md#engine-instance-seam). Use its checker when both layers contain the same script basename.
