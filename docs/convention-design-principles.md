...
| 2026-09-18 | §8.12d に 3 つ目の形「設計記録に『手作業で』 を置く」 (#manual-work-in-design-records) を追記 = 既存分の機械移行 (行単位 + 読み直し照合) と commit 時の停止までを 1 単位に | layer-3 の schema 変更で「既存分は手動移行」 と記録したまま、 件数表示だけの lint が 4 か月で 37 → 187 件に増えた (実測) |
