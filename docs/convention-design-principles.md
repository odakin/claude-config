...
3. **README は thin index に降格する**: 定義本体を README から抜き、 home への pointer だけ残す。 ⚠️ これは「README は SoT たりえない」 を含意しない: **外部 contributor のいる OSS では build/quickstart/deploy の home は README 側**なので、 降格・抜き出しの対象は README でなく CLAUDE.md 側になる (= どちらが home かは [`CONVENTIONS.md` §README の流儀](../CONVENTIONS.md#readme-style) の判別軸で決まる、 「README=非SoT」 は内部リポ default にすぎない)。 ただし **外部 (= 編集権限のない別 repo) が指している anchor / heading は保存し、 path を rename しない** (= path-targeting な外部 ref を dangle させない。 §14.2 の legacy 保存原則の cross-file 版)。
