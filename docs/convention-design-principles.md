...
- **agent 別の写しも variant**: 同じ規約 file を別の agent 用の入口として丸ごと写し、 「片方を直したら両方直す」 と注記する形。 注記は carrier ではないので片方が古くなるうえ、 **写しを製品名の一括置換で作ると本文の語まで書き換わる** (= 別製品の名前を説明していた文が、 置換後は嘘になる)。 入口が要るだけなら写さず、 読めと言うだけの薄い入口にする (層1 の入口契約と同じ形)。
...
| 日付 | 変更 | 動機 |
|------|------|------|
| 2026-09-18 | §8.66 新設「検査が列挙する単位を、 規則が守る対象の単位に合わせる」 / §21.1 に agent 別の写しの form / §8.12d に 4 つ目の形 (規則を場所で書く) | 規則は「agent が task を始める場所」 を対象にしていたのに検査が repo を列挙しており、 容れ物の dir だけが素通りしていた (実測)。 同じ整備で、 規約 file を別 agent 用に丸ごと写した入口 (置換で本文の語が壊れていた) と、 場所で書いた push 規則が入口の整備まで止めていた形も出た |
