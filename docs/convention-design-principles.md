...
- <a id="relabel-derived-from-key"></a>**機械が書く派生値 (索引の名前・向き・分類の label) は key を正本にし、 規則を変えたら key から引き直す道具で揃える**。 派生値を手で直すと、 次に規則を変えた時にまた手で直すことになる。 引き直す道具は冪等にし、 dry-run で「今の規則とずれている件数」 を全件数えてから書く (= dry-run がそのまま監査になる)。 規則の候補が複数あるなら、 書き換える前に全件で候補ごとの外れを数えて選ぶ (実測: 返事かどうかを mail の header で判定する案は、 送信の道具がその header を付けない・直前の自分の message を指す ために外れが最も多く、 thread の中の順序で判定する案が最も外れなかった)。 実装例 = [`conventions/mail-thread-ledger.md`](../conventions/mail-thread-ledger.md) の `--relabel`

<a id="automate-before-asking-others"></a>**5 つ目の形 = 他人への依頼を先に書く** — 共同編集者や事務に「今後は◯◯してください」 と運用の変更を頼むメールを、 機械の側を作る前に書く形。 相手の記憶と手作業に新しい手順を足すだけで、 守られなかったときに気づく仕組みも無い。 ∴ **先に相手の手元で自動で動く仕組みを実装し、 本物で動くことを確かめてから、 メールは「こうしました、 不具合があれば教えてください」 の報告にする** (= 相手に頼むのは了解だけ。 「〜したいです。 代わりに…」 のような依頼の形にしない)。 相手の手元で動かすには、 発火を相手の操作 (例: push) そのものに置き、 仕組みは相手の環境で単体で動く写しとして repo に置く。 自分の側の別マシンへの配線は §1〜2 の自動適用に載せる (実測: 運用の変更を頼む下書きを「先に自動化を実装して了解だけ取る」 形に作り直すと、 相手の作業は 0 になり、 文面は 3 行になった)。
...
<a id="parity-against-own-output"></a>**変種 (2026-09-24 追記) — 照合の基準に前の出力を使うと、 前の欠けを正しいと認定する**: 生成の経路を作り直す・移す・リファクタするとき、 「新しい経路の出力が旧経路の出力と一致する」 ことを合格の条件にしがちである (一致は安く測れ、 差が出れば説明を求められる)。 旧経路が何かを**黙って落としていた**なら、 旧出力にもそれは無く、 一致の検査は欠けを**仕様として焼き直す**。 検査の基準は同じ経路の過去の出力でなく**独立した参照** (配布雛形そのもの・一次資料・別経路で作った出力) から取る。 型は 3 つが揃うと閉じる: ① 検査が**書いたもの** (記入値・差分) しか見ず、 **元から在るべきもの** (雛形が持つ見出し・枠・記号) を出力で確かめる段が無い、 ② 目視は在るものに目が行き、 無いものは比べる参照が無いと見えない (「前と同じに見える」 は前も欠けていれば通る)、 ③ 設計の前提 (「その部分は紙に出ない」) が、 欠けた出力を見て書かれ、 検証されずに残る。 対策は ① を機械にする = 基準を雛形から機械で作り、 出力と突き合わせる段を置く (様式の図形の字 = [`scripts/check-form-static-text.py`](../scripts/check-form-static-text.py))。 兄弟 = [§8.16](#absence-channel-coverage) (不在の主張は見た面の外を保証しない)。

origin: 様式の生成経路の移行で、 新 recipe を旧 driver の出力と「語が同一」 で照合して合格にした。 旧 driver は見出しの図形を落としていたので、 見出しの欠けが仕様として引き継がれ、 engine の説明にも「その様式の図形は元から紙に出ない」 と書かれた (雛形の図形を列挙した直後に)。 同じ機構の兄弟の症状 (選択の箱の消失・紙の上で手書きで補われた欄) は先に別々に見つかっていたが、 それぞれ見つかった症状だけを直す RCA で閉じていた。
...
6. <a id="text-section-refs"></a>**文中の「path §「節名」」 は link でないので、 (C) の検出器にも anchor 検査 (3) にも見えない — 実在する file の実在しない節を指す参照を別に見る** (2026-09-24 追加) — (B) ② の書き方 (file 名 + 見出しテキスト) は、 読み手が file を開いて節名で探すことを前提にしている。 節が別の file にあると、 探した結果は null になる。 実測: 雛形の「`<dir>/README.md` §「<節名>」」 の節は同じ dir の別 file にあり、 その雛形で文面を起草した session は README を節名で探して何も得ず、 手元の前例と古い記述に倒れて誤った手順の文面を作った。 fleet の文中参照 約 9,600 件 (§「名」・§名・文中の `path#anchor`) に当てた初回は 🔴 537 件で、 大半が検出器側の誤りだった (祖先 dir の解決漏れ・大小文字を無視した後方一致・行番号の anchor・各 repo にある同名 file・移設 stub・link 文字列の中の §「…」・code の fixture)。 誤りを直した後に残った型 = 節の archive への移設・doc の分割・節の改名・雛形の古い pointer。 道具 = [`scripts/check-text-section-refs.py`](../scripts/check-text-section-refs.py): **file が 1 つに決まったのに節が無いもの (🔴) と、 file が 1 つに決まらないもの (⚪) を同じ値にしない** ([#catch-all-branch-absorbs-covered-class](#catch-all-branch-absorbs-covered-class))。 同名の別 file にあるもの (文脈で別 repo の CLAUDE.md を指す等) は 🟠 に分け、 履歴の記録 (archive・inbox・threads) の中は 📜 で数える (書いた時点の事実)。 🔴 には、 その節名を見出しに持つ file を候補として付ける。 発火面 = commit 時 (追加した行の参照 + この commit で消した見出しを指す他 file、 警告を出して commit は通す) と fleet scan。 **書く側の規律**: 節への参照は `path#anchor` の link にする (link は (3) の検査に入る)。 節名だけで指すときは見出しの文字列をそのまま写す (要約した名前は grep で当たらない)。 節を別 file へ移したら、 その節名を文中で指す参照を同じ commit で直す (commit 時の警告がその一覧を出す)。
   - <a id="text-section-refs-code-and-tracing"></a>**code の注釈も source に入れる / 直し先は履歴で決める** (追記) — 設計 doc を分割すると、 prose の参照は直されても code の注釈に分割前の節参照が残る (実測)。 検出器は注釈と docstring を読み、 test の fixture は「行内で閉じる文字列 literal」 だけで除く — 行頭からの引用符の数の偶奇で literal を判定すると、 注釈の英文の apostrophe や docstring の backtick で本物の参照を取りこぼす。 **直し先の決め方**: 見出しの完全一致で見つからない節は `git log -S'<節名>'` と `git log -G'^#+ .*<語>'` で改名・統合・分割の commit を辿り、 行き先が 1 つに決まるときだけ直す。 見出しになったことの無い名前 (commit message の設計項目の名前など) は、 節参照をやめてその commit を指す。 節の中の番号つきの項 (「原則 3」) は並びが変わると黙ってずれる ((C) の positional `§N.M` と同じ型) = 番号を写し替えず、 参照元の文が今の doc のどの項に当たるかで決める。 鉤括弧なしの `§名` は境界が推定なので検査が緩い (先頭の語が本文にあれば通る) = 書くときは鉤括弧か link にする。 検出器の述語を変えたら、 変える前の `--json` を取っておき `--compare` で fleet の差を 1 件ずつ見る ([#catch-all-branch-absorbs-covered-class](#catch-all-branch-absorbs-covered-class) の「直す前の実装で陽性対照を赤にしてから直す」 と対)。
...
<a id="age-fold-hides-the-stalest"></a>**同じ逆転は「古い item を件数に畳む」 表示でも起きる**: 一覧を短くするために「N 日より古いものは件数だけ」 にすると、 **放置された依頼ほど見出しから消える** — 古いのは誰も動いていないからで、 急ぐ理由が増えこそすれ減ってはいない。 相手の番を待つ依頼 (掲示板の引受待ち・検収待ちなど) では、 古さはそのまま相手を待たせている時間。 短くするなら古い側を畳まず、 全件を出して古さを印にする (例: 「放置」)。 畳んでよいのは、 期限も相手の待ちも無い参考の行だけ。 また、 一覧の元を読めなかった (lock・破損・未同期) ことを「番が無い」 と同じ空の表示にしない ([§22.3](#false-healthy-pattern) の 2)。

origin: 期限の短い依頼が、 窓の間ずっと無印・一覧の下段にあり、 🚨 と強制 disposition が窓の後にしか付かなかった (実測)。 件数への畳み込みは、 session 開始時の一覧が 14 日より古い依頼を件数に畳み、 最も長く放置された依頼が見出しに出ていなかった (実測)。
...
6. **網の手順の一覧は、 手順を定める全 doc から棚卸しする — 期日の表 1 つから写さない**。 期日の表が 1 か所にあっても、
   規約の本文 (後の漏れの対策で足した「毎回 N 日前までに…」) に表が追随していないことがある。 表だけを入力にして網を作ると、
   **本文にしか期日が無い手順だけが網から落ちる**。 棚卸しは表・規約の本文・手引・案内の雛形を横に並べ、 本文と表の食い違い
   (本文 = 期日つき / 表 = 期日なし・「〜の後」 の event 駆動) を見つけたら表を直してから網に入れる。 別の手引に表の写しを置かない
   (写しは正本に追随しない)。 逆向きも同じ: **手順に期日を足した turn に、 表と網にも足す**
7. **「毎回、 期日つきの独立の記録を立てる」 を漏れの対策にしない** — それは起票の規律の再生産で、 次の新しい回の起票で落ちる
   (対策の turn に、 その時点で見えていた回にだけ遡って起票され、 以後に確定した回には一度も立たない形になりやすい)。
   同じく「その回の記録があるか」 を見る検査は起票依存の二次形で、 記録の名前の付け方に縛られて誤報し、 誤報が多いほど読まれない。
   漏れの対策は 1. (行事の日付から導く) と 2. (行為の記録で見る) の形で入れる
8. **後始末の経路を「済んだ」 に数えない**。 手順が回らなかった時の fallback (本来は相手に事前に送る物を、 こちらが当日に用意する) が
   規約に正規の経路として残っていると、 漏れは当日まで表に出ず、 fallback を使った回も完了として記録されやすい。
   網は fallback の状態 (こちらで用意した) を本来の手順の完了に数えない
9. <a id="deadline-is-not-act-date"></a>**期日は「遅くともここまで」 であって、 出す日ではない**。 受け手 (事務の窓口) が早めを望む手順や、
   材料がそろえばすぐ出せる手順は、 **きっかけ (行事の確定・材料の到着) の時点で「今できる」 として出し**、 期日は超過の判定だけに使う。
   「期日の N 日前から出す」 設計は、 期日を事実上の実行日にする (実測 = 〆の直前か超過で出る回が続いた)。
   「今できる」 には**材料がそろっている条件**を付ける (例: 書類の記入に要る登録値・財源が決まっている期間)。 条件の外まで
   「今できる」 と出すと、 できない手順が毎朝並んで読まれなくなる。 遠い回まで出すなら、 出す範囲 (何日先まで) も材料の条件に合わせる。
   手順書の側も同じ turn に「〆」 と「出す時期」 を分けて書き直す (「N 日前」 の一語が両方を兼ねていると、 読み手は〆を出す日と読む)
...
origin (6.〜8.): この網を入れた直後の回で、 講演者に事前に送って当日持参してもらう書類 (こちらが当日に刷る運用の「刷り忘れ」 を
根治するために導入した手順) が抜け、 当日にこちらで作って刷った (実測)。 その手順の期日は以前の漏れの対策で規約の本文に足されていたが、
期日の表と、 手引に置かれた表の写しは「別の書類の後」 の event 駆動のまま追随しておらず、 網の手順の一覧はその表から作られていた。
以前の対策は「回ごとに期日つきの独立の記録を立てる」 と「その記録があるかを見る検査 (一覧の画面にだけ出る)」 で、 記録は対策の時点で
見えていた回にだけ遡って立ち、 後から確定した回には立たず、 検査は記録の名前の付け方の違いで誤報を出していた。 講演者への案内の雛形は、
先頭の本文が旧運用の持参物 (印鑑だけ) のまま、 新運用の形が後段の変形として置かれていた。
さらに雛形の「詳細は <file> の §<節名>」 が実在しない場所を指していて、 案内を起草した session はそこを探して何も得られず (null)、
手元の前例と旧運用のままの記録の文言に倒れた (実測。 文中の参照は markdown link でないので、 anchor の検査に掛からない)。
...
### <a id="fixture-default-bakes-bug"></a>8.73 selftest の fixture の既定値が現実と違う軸は、 その軸に依存する不具合を期待値に焼き込む — helper の既定は現実の向きから取る

fixture を組む helper (`msg(id, ts, from, ...)` のような関数) は、 書く手間を減らすために多くの field に既定値を持つ。 その既定値が現実と違う軸 (例: どの message も宛先 = 自分) では、 その軸を読む実装が現実と違う値を出しても fixture の上では区別がつかず、 **誤った出力がそのまま期待値として書かれる**。 test は緑のまま、 不具合を仕様として固定する。

実測の形: mail の記録道具が、 自分発の message の相手に差出人 (= 自分) を書いていた。 selftest の helper は全 message の宛先を自分にしていたので、 「相手 = 差出人」 と「相手 = 宛先」 が fixture の上で同じ値になり、 期待値には自分の名前が入っていた (気づいたのは利用者)。

- **helper の既定値は現実の向きから取る**: 他の field から現実の値が決まる軸 (自分発なら宛先は相手、 相手発なら宛先は自分) は、 既定値もそれに従わせる。 固定値の既定を置くのは、 実装がその軸を読まないと確かめた軸に限る。
- **期待値を書く瞬間に、 その値が現実のデータで出るかを 1 回問う**: 期待値の中に現実には起きない値 (自分宛ての返信・自分を相手にした現在地) が無いか。
- **直したら、 直す前の実装で赤くなることを確かめる** ([§8.70](#isolation-shortens-the-path))。 fixture を直しても旧実装が緑なら、 その test はまだ軸を見ていない。
- 姉妹: [§8.67](#green-check-endorses-unasked-premise) は「期待値を、 成果物を作った手順から写さない」、 本節は「期待値を生む fixture そのものが現実と違う」。 [`#slug-detector-pinned-to-renderer`](#slug-detector-pinned-to-renderer) は「近似した検出器は、 renderer が実際に出す値で fixture を組む」。

origin: 記録道具の selftest で、 自分発の message の相手が期待値ごと誤っていた (実測)。

### <a id="directive-strength-before-cannot"></a>8.74 組織の文書の「〜しないように」 を「できない」 と伝える前に、 文の強さと例外の手続きを確かめる — お願い・手続きつきの可・禁止は別物

組織の依頼文・通知には、 強さの違う書き方が混ざる: **禁止** (規程の条文。 「〜してはならない」) / **お願い** (「〜しないようご留意ください」「原則として」) / **手続きつきの可** (「万一〜する場合は、 〜を提出してください」)。 お願いや手続きつきの可を読んで「できない」 と伝えると、 user は取れる選択肢を 1 つ黙って失う。 しかも user は文書を読んでいないことが多く、 誤りに気づく手がかりが無い。

- **同じ束の別の文書が例外の手続きを持つ**: 同じ時期に配られた文書の束は、 1 通がお願いだけを書き、 別の 1 通が例外の手続きを書く、 という分担になっていることがある (実測: 取りまとめ役宛の依頼文に申請の手続きがあり、 同梱の通知にはお願いだけがあった)。 **「できない」 と言う前に、 束の全部を「万一」「場合は」「申請」「変更」「除き」 で引く**
- **例外の理由が自分の状況に当たるかを見る**: 「〜の理由を除き」 と書かれた例外の理由が、 手元の別の制度 (減担・休暇など) の手順の中で同じ操作として書かれていれば、 手続きすら要らない可能性がある。 当たるかが文面で決まらなければ、 そう書く
- **伝え方は段を分ける**: 「避けるよう求められている (お願い) / 手続き X を踏めばできる / 理由 Y に当たるなら手続きも要らない可能性」。 規程本体の条文を読んでいなければ、 読んでいないと書いて判断を user に渡す (= [§8.36](#input-does-not-transfer-decision-ownership) の「事実・規程適用・将来確約を分離する」 の、 規程の強さの軸)
- 姉妹: [§8.16](#absence-channel-coverage) は「告知が無い」 の不在主張を 1 つの経路で決めない。 本節は「できない」 という否定の主張を 1 通の文書で決めない

origin: 組織の依頼文のお願いの一文を読んで user に「できない」 と答え、 同じ束の別の文書に変更の手続きがあった (実測)。

### <a id="control-dependency-is-dependency"></a>8.75 制御の依存も依存 — 前の結果で次を出すか決まる操作は、 同じ応答に並べない

agent は 1 つの応答に複数の tool call を並べられるが、 応答の途中で前の call の結果は読めない。 「依存があるなら前の結果を待て」 という指示は、 ふつう**値の依存** (前の結果を次の引数に使う) として読まれる。 **制御の依存** (前の結果で、 次を出すか・どれを出すかが決まる) は引数に現れないので、 独立に見えて並べられやすい。 典型 = 取り消し → 作り直し / 投稿 → それを指す通知 / 検証 → 公開。 並べると、 前の call が失敗や想定外を返しても、 後ろの call は既に出ている。

- **規則**: 後ろの call を出すかどうかが前の結果に依るなら、 前を単独で出し、 結果を読んだ次の応答で決める。 計画を「A が成功したら B」 と言える形で書いた時点で、 それは制御の依存
- **相手が速い操作は、 取り消せる前提を置かない**: 人や別の process がすぐ拾う操作 (起動の提案・予約送信・監視される queue への投入) は、 作った瞬間に相手が動く。 取り消しは「間に合えば幸運」 と扱い、 訂正は相手への連絡で行う。 作り直すのは取り下げの成功を読んだ後だけ
- **変わりうる事実を相手に渡す物へ写さない**: 渡した物が古くなると「作り直し」 が要るように見え、 上の 2 つを誘う。 渡すのは宛先の id・正本の path のような変わらない鍵だけにし、 変わる事実は正本に置く
- **機械の網**: 同じ依頼の 2 つ目の起動を、 起票の直前に「この session が出した取り消し未確認の同じ依頼」 と「同じ依頼で稼働中の相手」 で止める。 規則だけでは、 並べた瞬間には効かない (結果を読む機会が無い)
- 姉妹: [§8.69](#failure-exit-equals-violation-exit) は「止まった」 の読み違い、 本節は「止められたはず」 の読み違い — どちらも、 見えていない結果を見たことにしている

origin: 取り消しと同じ依頼の作り直しを 1 つの応答に並べ、 取り消しが「既に起動済み」 を返したときには 2 つ目も起動していた (実測)。 起動の提案は作成から十数秒で人に押されていた。
...
| 2026-09-23 | §8.16 に「人のアドレスを 1 つの経路だけで数える」 変種 (#person-address-channels) と、 共有サービスへの招待は見つかったアドレス全部に送る、 を追加 | 共有フォルダへの招待先を受信メールの送信元だけで集めて「これで全部」 と言い、 予定の参加者にあったアドレスを落とした |
| 2026-09-23 | §8.55 に「古い item を件数に畳む表示でも同じ逆転が起きる」 (#age-fold-hides-the-stalest) を追加 | session 開始時の一覧が古い依頼を件数に畳み、 最も長く放置された依頼が見出しに出ていなかった |
