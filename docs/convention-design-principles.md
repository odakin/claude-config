...
| 2026-09-14 | §8.51 新設「検出器に鳴らされた文が欠陥とは限らない — 規約どおりの使用なら、直すのは検出器で、原文ではない」 | 正本の重複検出が語の使用に鳴った箇所を、 対処文どおりに本文側で直そうとして原文を書き換えた (差し戻して検出器の目印を定義文へ移した)。 対処文が本文の変更しか示していなかった |
