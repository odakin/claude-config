...
- **上限値が実額の欄に居座る**: 「受給額 = 500 万円以下」 — これは**公募の上限**であって受給額ではない。 交付決定 (= 実額) が出た後もそのまま残り、 別の申請書の「研究費取得状況」 欄に**過大申告**として転記されかける。
- **暫定額ベースの派生値が更新されない**: 「費目間流用の承認不要枠 = ±1,923,000 円 (= **応募額**の 50%)」。 交付決定で総額が下がったので正しくは ±1,500,000 円。 **過大枠を信じて流用すると事前承認の手続漏れになる**。 ⚠️ この doc は「**確定額が出たら再計算**」 と自分で書いていた — 指示は landing したが、 実行されなかった。

**規律**:

1. **暫定値には出所と失効条件を同じ行に書く** (= 「±1,923,000 (= **応募額** 3,846,000 の 50%。 ⚠️ 交付決定で要再計算)」)。 出所を書かない数値は、 後から見て確定値と区別できない。
