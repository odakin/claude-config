...
## SESSION 索引 09-12 (2026-09-15 退避)
...
- **09-13 link の壊れ方を発生の瞬間に止める + 前日の誤った正本を訂正** (owner「まかせるのでぜんぶ一番いいように」): (1) **commit 時 guard** = [`fix-md-links.py`](scripts/fix-md-links.py) `--staged` (stage した `.md` の **index の中身**を 0.1 秒で検査) を個人層 pre-commit chain (54 repo) と [`public-precommit-runner.sh`](scripts/public-precommit-runner.sh) (公開 repo) の両方に配線。 段数ずれを commit しようとすると止めて 1 行の修正コマンドを出す。 ⚠️ 設計の要点 = **exit code だけで止めない** (engine の構文エラーも exit 1 = 壊れた engine が全 repo の commit を止める) → 「exit 1 かつ finding の見出し」 の時だけ止め、 内部失敗は exit 3 で WARN。 正本 = [`hook-authoring.md#engine-failure-must-not-block`](conventions/hook-authoring.md#engine-failure-must-not-block)、 runner test 27/0 (壊れた engine の foil は個人層 e2e test 側)。 (2) 発生源の正本手順 [`CONVENTIONS.md#session-trim-anchor-preservation`](CONVENTIONS.md#session-trim-anchor-preservation) に 4 つ目の型「移した本文の相対 link の段数ずれ」 と修正コマンドを追加。 (3) 検出器の偽陽性をもう 1 型修正 = **list 内の字下げ fence** を fence と認識していなかった (例示の除外 list が要らなくなった)。 (4) **前日に書いた正本の誤りを訂正** = [`latex.md#align-split-tag-orphan`](conventions/latex.md#align-split-tag-orphan) は著者裁定に反して式番号の配置を欠陥扱いしていた。 fleet 状態 = anchor 2916 本全解決、 着地先の無い相対 link は直せる型 0 (残りは数式・件名の誤認と、 公開時に作る前提の下書き画像 2 本)。

- **09-13 着地先 file の無い相対 link を fleet で掃除 + 道具化** (owner「直せるものはぜんぶ直して！」 の続き): anchor 検査の射程外 = 着地先 file そのものが無い link を素朴に測ると 898 本 (うち 457 本は `~/Claude/CLAUDE.md` 等の **symlink の複製を symlink の dir から解決**した測定側の誤り)、**renderer と同じ解決**にすると 8,718 本中 505 本。型は 4 つ = 段数ずれ (SESSION-archive/ 等へ MOVE して `../` を張り直していない) / git が記録した改名・一意な移動先 (過去の記録が削除済み file を指すものは削除直前の permalink) / 数式・メール件名の誤認 / 後継の無い削除。**修正は 255 箇所 = 段数ずれ 219 + 改名・移動 31 + permalink 5** (commit 済み差分から実測)。報告のみ = 数式・件名の誤認と後継の無い削除。**13 repo・47 file**、全 commit を「差は link target 内だけ・新しい着地先は実在・対象ごとに差が在る」 で事後検証。道具 = [`fix-md-links.py`](scripts/fix-md-links.py) (分類 + `--fix` + 自己検証、suite §2b に `--strict` 配線)、`check-md-anchors.py` も同じ解決に揃えた。正本 = [`#link-target-rot`](docs/convention-design-principles.md#link-target-rot)。⚠️ **踏んだ罠 3 つ** (全部 selftest の foil): 前方一致の置換が正しい link を壊す / 例示の中まで書き換える / **非 ASCII の path を `git diff --name-only` で取ると quote されて検証が空振りで PASS** (2 file が未検証のまま「検証済」) → [`debugging-discipline.md#tooling-that-lies-on-binary-and-non-ascii`](conventions/debugging-discipline.md#tooling-that-lies-on-binary-and-non-ascii) に行を追加。並行 session が編集中の repo は「書換え → 検証 → 自分の path だけ commit」 を 1 回の実行にし、相手が追記しうる当月 archive は相手の commit 後に適用 (実際に相手の commit がその file を含んでいた)。

- **09-13 壊れた anchor を fleet 全体で 0 に + 検出器を GitHub の slug 実装に固定** (owner「直せるものはぜんぶ直して」): [`check-md-anchors.py`](scripts/check-md-anchors.py) を fleet に当てた初回は 33 件、うち **19 件が検出器側の誤り**だった = (a) 空白の連続を `-` 1 本に潰していた (github-slugger は 1 文字 1 本) / (b) 見出し中の `[label](url)` の URL まで slug に混ぜていた / (c) code span・fence 内の構文例・`#L53` 行参照・`<slug>` を link と数えていた。正本 = [`#slug-detector-pinned-to-renderer`](docs/convention-design-principles.md#slug-detector-pinned-to-renderer)。slug 計算は `check-inbound-refs.py` の `gh_slug` + `rendered_heading_text` に 1 実装で置き両検出器が共有、selftest に名指し fixture と過剰除外の foil。**本物 13 件**は path 修正 (別 doc の節を同一 file 内 `#` で指していた 6 件)・転送先補正・明示 id・`%5C` 除去で解消 → **3001 link / 未解決 0**。回帰防止に [`run-all-checks.sh`](scripts/run-all-checks.sh) §2b へ配線 (`--base "$ROOT"` = CI に兄弟 repo が無くても着地先の無い link は skip)。同時に inbound 検査の informational「code-file dangling 12」 を解消 (全件が selftest の `foo.md` fixture = 常に出る行は読み飛ばしを訓練する)。positional 参照 1206 件は §14.7 (B) が許容と決めた形なので対象外。

- **09-12 分割 6 日後の参照腐食を 3 kernel + 1 script に (= 検出器が偽陽性で埋まり、壊れた自己参照は誰も見ていなかった)**: 記法改稿の hoist 中に `check-inbound-refs.py` が **HARD dangling 29 件**を報告したので path 解決で真偽を判定したところ、**全件が偽陽性**だった (2026-09-06 の layer-1 分割で同名 doc が 2 箇所に在り、新 home へ正しく path で指す ref が stub の anchor 集合と照合されていた)。正本 = [`#basename-index-collides-after-split`](docs/convention-design-principles.md#basename-index-collides-after-split) — **明示 path を持つ ref は path で解決してから basename 判定に落とす** (修正済、29 → 1。残る 1 は本当に間違っていた path で、これも同 session で作った行)。⚠️ 同時に見つかった第 2 の腐食 = [`#forwarding-stub-is-a-snapshot`](docs/convention-design-principles.md#forwarding-stub-is-a-snapshot): **転送 stub は分割時点の anchor しか持たない**ので、移設後に新 home へ足した節を名前で指す ref は最初から壊れ、節を足すたびに単調増加する (実測 6 日で 4 anchor / 19 ref)。stub 3 本に転送行を追加し、実在しない `#parallel-sessions-same-repo` を `#staging-window-race` に訂正。第 3 = [`#self-reference-is-nobodys-business`](docs/convention-design-principles.md#self-reference-is-nobodys-business): inbound 検査は設計上 target repo の内部 ref を除外するので、**同じ file の中の壊れた `#anchor`** は誰も見ていない → [`check-md-anchors.py`](scripts/check-md-anchors.py) (path で全 link を解決、slug 計算は `check-inbound-refs.py` の `gh_slug` を import して 2 実装にしない)。config 3 repo の 2487 link に **21 件**検出、うち単複 typo 1 件 (`#proxy-blind-spots`) を修正、残りは未手当 (README の TOC 自己 link・消えた見出し・doc 中の placeholder)。

- **09-12 記法の一括改稿を盲検検査させた回の hoist (組版・記法側)**: 「機械的な置換だけ」 と主張する改稿の検査で出た知見のうち、LaTeX と原稿 audit に属する分。[`latex.md#align-split-tag-orphan`](conventions/latex.md#align-split-tag-orphan) = ~~式番号が 3 行目に落ちるのを直す~~ → **09-13 訂正: 長い式の番号が下の行に回るのは amsmath の標準で欠陥ではない** (著者裁定「完全にカノニカル」、 査読で should-fix にしない・番号を戻すために式の割り方を変えない)。[`#symbol-width-repagination`](conventions/latex.md#symbol-width-repagination) = **記号幅が変わる一括置換は総頁数が同じまま頁割りだけ動かす** — 頁数は総和なので前半が縮み後半が伸びると相殺する。∴ `.aux` の label→(番号, 頁) を版間で比べる (実測: 総頁数不変のまま label 5 本が頁移動、うち 1 本は後ろへ = 人手の頁 grep が落としていた)。[`paper-audit.md#notation-rename-sweep`](conventions/paper-audit.md#notation-rename-sweep) = 機械が純粋性を通した**後に人が読む 4 点** (初出での scheme 定義 / 2 綴りの一致箇所 / **区切り記号の衝突は改稿によって初めて生まれる** / 改訂しなかった節にこそ別綴りが残る)。道具と kernel の正本は層1 = [`ai-collaboration/scripts/`](../ai-collaboration/scripts/README.md) の `check-rename-purity.py` / `compare-tex-builds.py` と [`physics-verification-cycle.md#referee-side-kernels`](../ai-collaboration/conventions/physics-verification-cycle.md#referee-side-kernels) 第 6 回。
...
owner の科研費 session (別 repo が主戦場) から、層1 に上がった分。過去の赤入れを
...
owner の物理 repo で規格化因子のフラグを監査したところ、 フラグの射程内は正しく、 代わりに **既に印字された式から写した前係数が定数倍ずれている**ことが判明した。 一般則を 2 本 hoist。
...
- **`conventions/paper-audit.md#composite-quantity-notation-migration` (新設)**: 合成量には「和」 系
  ...
  2 倍ずつ同じ向きに効いて括弧の中で 4 倍が落ち、 それを引いた式にも伝播。
...
**2026-06-10 (office-automation.md に 3 gotcha 追記 + tool-call-robustness.md に ENOSPC 失敗モード)**: 前 session 群 (官製様式の連続作業) の handoff plan に基づく cold-eyes audit で、 既存 slug 未カバーの 3 件を layer 1 化。 (1) [`merged-cell-write-topleft`](conventions/office-automation.md#merged-cell-write-topleft) に **別 sheet で同項目の座標が違う → 流用すると merged の非 top-left を指し `MergedCell` read-only で silent fail** (= sheet ごとに dump し直す) を追記 (`33976cf`)。 (2) [`openpyxl-destroys-drawings`](conventions/office-automation.md#openpyxl-destroys-drawings) 回避2 (drawing XML zip 注入) に **別テンプレ注入時の安全 3 前提** (merged 範囲一致 / standalone=media 非参照 / rId 非衝突) を肉付け (`461ed5e`)。 (3) [`datetime-cell-hash-overflow`](conventions/office-automation.md#datetime-cell-hash-overflow) に **`###` overflow を PDF text 抽出で検出するのは renderer 依存で不定 (値/化け/`###` literal) → PNG 目視で確定** を追記 (`d63d00c`)、 直後の deep 4 軸 check (無矛盾性軸) で初版の「text 層に入るのは元の値」 断定が renderer 依存を見落としていたのを訂正 (`60fb9fe`)。 別途 [`tool-call-robustness.md`](conventions/tool-call-robustness.md) に **「出力 capture の ENOSPC」 節**新設 = malformed (model serialization bug) とは別の失敗で、 harness の Bash stdout/stderr capture fs が満杯だと出力が失われる (= メインディスク空きと独立、 コマンド自体は実行され得る) → 出力を file に redirect して Read する workaround (`9235fd3`)。 index validator dangling0/orphan0、 公開追加行を grep して email/uid/UUID/private repo 名 0 hit を実バイト確認。 全 generic (人名/機関名/private repo 名なし)、 process 側 (どの form) + sot-registry の malformed 真因 anchor 登録は layer 3 (odakin-prefs) に分離。
...
**2026-06-05e (office-automation.md に「多 sheet xlsx → 提出用 PDF」 RECIPE + 新 slug 8 個)**: 複数 sheet の様式 xlsx を提出用 PDF にする end-to-end の作り込み知見を layer 1 化 (= origin: 官製様式の PDF 業務、 週次化に向け user が「完璧に再現できる手引書化」 を指示)。 新 H2 [`multi-sheet-pdf-assembly`](conventions/office-automation.md#multi-sheet-pdf-assembly) = **RECIPE** (構造把握→fill→ページ体裁→出力→検証→永続化、 既存+新 slug を task 順に連結) + 新 slug 8: `excel-pdf-whole-workbook-export` (= Excel の save-as は workbook 全体を出力→不要 sheet を temp 削除、 数式参照先は残す #REF 回避) / `one-sheet-multi-page-split` (= print_area 切替→個別出力→fitz 結合) / `print-area-bbox-fit` (= 実内容 bbox に詰める、 広いと fitToHeight で縮小+余白偏り) / `even-margins-centering` (= page_margins 統一+水平/垂直中央寄せ、 Excel 尊重 / soffice は verticalCentered 無視) / `fitz-pdf-toolkit` (= render/crop/結合/page_count) / `pdf-margin-pixel-measure` (= 非白 bbox で余白を px 実測) / `soffice-excel-fidelity` (= engine 体裁差 table) / `trailing-print-artifact` (= 表末尾の浮いた点線=余分末尾行)。 index.yaml に 9 entry 追加 + validator dangling0/orphan0 (95 sections)。 top に RECIPE 入口 pointer + stale な「Excel=1 sheet」 表記に caveat。 別 commit で `merged-cell-write-topleft` に罫線 anchor 追記済。 全 generic (人名/grant 番号/機関名なし)。 process 側 (= 様式 fill→検証 手順) は private repo の runbook に分離 (= layer 分割)。
...
**2026-06-02 (office-automation.md §5-7 / §5-8 新設: 選択肢ラベルの選択マーク + multi-sheet 数式伝播)**: 官製様式 fill の差戻し事例から 2 つの一般則を §5 に追加。 §5-7 = **選択肢が pre-printed label として並び専用 input cell が無い form** での選択マーク: ○を**前置き** (`"○出張者立替"` = label 保持) は OK、 `☑` 単体で label を**置換** (= 破壊) は NG。 境界線は「破壊か保持か」 であって「label cell に触るな」 ではない (= prohibition を過度一般化すると有効な選択マークまで禁じる)。 ☐ checkbox cell を持つ form は `☐→☑` toggle で別扱い。 ○前置きは filled value が template label を substring 包含するので §9 の検出漏れ pattern を手 review で補完。 §5-8 = multi-sheet form の **数式伝播** (= 主 sheet source cell のみ fix で従 sheet 自動反映、 二重入力しない) + **literal の帰属区別** (= 同一文字列でも 出張者 vs 依頼者/機関 で帰属違い、 一括置換禁止) + **審査機関の明示指定 > form 内蔵の例示語**。 layer 1 public のため人名 / 機関名 / 個別 form data は排し汎用フォーム用語のみ。 origin = 官製様式 ☑ 上書き差戻し → ○前置き修正 (= 既存 §9 の「短い header label 上書き検出漏れ」 と同 form・補完関係)。 layer 2 (= 事務 repo) 側に form 固有手順 + worked example を別途記録済 (= layer1→layer3/2 参照せず汎用則のみ layer 1 化)。 直後の 4 軸 sweep (無矛盾性軸) で **§5-4 step2 の絶対則「label 行 write 禁止」 が §5-7「○前置き OK」 と節間で緊張** (和解文が §5-7 内のみ) を発見 → §5-4 step2 に §5-7 への carve-out を追記 (= 禁止の本体は「label 文字の消失」、 「label cell に一切触るな」 ではない を明示)。
...
## SESSION 索引 09-06〜09-09 (2026-09-15 退避)
...
## SESSION 索引 09-10 (2026-09-15 退避)
...
## SESSION 索引 09-11 (2026-09-15 退避)
