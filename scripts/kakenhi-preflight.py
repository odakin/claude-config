...
組み直し運用ではその網が丸ごと外れる。実例: 様式のある欄の見出しと研究期間セルが消えた状態で
提出し、機関事務から様式の改変として差し戻された。**様式 docx にはその指示自体が印字されていた**
...
[PDF]   --pdf <調書.pdf>
  ...
  🟠 BARE_NUMBER       裸の括弧数字 (「（123）」= 被引用数の単位落ち)
...
# (実例: 事務が削除を求めた記入要領が提出版に残り、2 度の検証をすり抜けた)。
...
# と言うので、欄ごとに読む必要がある (実測: 空欄指定の欄に「該当しない。」と書いて
# 「トル / 空欄のまま提出するため」と赤字)。
BLANK_MARKERS = ("空欄のまま提出", "空欄で提出", "空欄のまま")
# 逆に「必ず書け」と様式が言っている印。空欄で出すと「記載して下さい」と赤字が入る
# (実測、相手機関の欄と所在地欄が空だった)。BLANK_MARKERS と排他 (空欄指定が優先)。
...
INSTRUCTION_PATTERNS = [
    ...
    #    実測: 空欄指定の欄に「該当しない。」と書いて「トル / 空欄のまま提出するため」と赤字。
...
# ── 表記 lint ────────────────────────────────────────────────────────────────
# 官公庁・機関の略称。調書は正式名称で書く (「文科省」は全件赤字指摘された)。
...
# 「（123）」型の裸の数字 = 単位・意味が落ちた数 (2026-09 に被引用数を括弧の数字だけで書いて赤字)。
...
# 「その他」に旅費相当が紛れる = 旅費との二重計上 (事務の指摘の例: 「滞在費は外国旅費に含まれるため削除」)
DOUBLE_COUNT_RE = re.compile(r"滞在費|宿泊費|渡航費|航空券|交通費|旅費")
# 逆向き: 旅費の行に「その他」費目のものが混ざる (事務の指摘の例: 「登録料はその他の費用に計上して下さい」)
...
def check_must_fill_sections(form: dict, pdf_text: str) -> list[tuple]:
    ...
    片方だけ見ても事故は防げない (実測: 空欄で出して「記載して下さい」と赤字)。
    """
    out = []
...
# 事務は「者？ 後述と表記のゆれ」と必ず突く (実測、同一頁で何度も)。
...
def check_identity(ident: dict, pdf_texts: list[tuple], csv_paths: list[Path],
    ...
    実際に起きた型: ① 科研費の機関番号 (5 桁) を e-Rad 機関コード欄と file 名に書いた
...
# 「月額 30 万円」「12 か月」「1 名」型の積算根拠 ([#keihi-meisai-expense-category])
...
# ── selftest (差し戻し実例を before/after で回帰させる) ─────────────
def selftest() -> int:
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        before.write_text(
            "費目区分,年度,品名・仕様,設置機関,事項,数量,単価,金額\n"
            "A,2027,数値計算用計算機,学内,,1,800,800\n"
            "B,2027,,,数式処理ソフトウェアライセンス,,,60\n"
            "C,2027,,,国内研究会での成果発表,,,180\n"
            "E,2027,,,英文校閲・RA謝金,,,70\n"
            "F,2027,,,研究会開催費（会場費・招聘者滞在費）,,,120\n"
            "F,2027,,,論文投稿・掲載料、クラウド計算資源利用料,,,100\n"
            "C,2027,,,国際共同研究の相互訪問に伴う研究打合せおよび成果発表のための出張旅費（年2回、各3泊4日、1名）,,,160\n",
            encoding="utf-8")
        ...
        after.write_text(
            "費目区分,年度,品名・仕様,設置機関,事項,数量,単価,金額\n"
            "A,2027,ワークステーション（解析用）・〈CPU〉/メモリ64GB 相当,○○大学,,1,800,800\n"
            "B,2027,,,専門書・文献,,,40\n"
            "C,2027,,,国内研究会での成果発表（年2回、各1泊2日、1名）,,,180\n"
            "D,2027,,,海外研究者の招聘旅費（北米から1名、5日間：航空券・滞在費）,,,160\n"
            "E,2027,,,データ整理補助のRA謝金（博士後期課程学生）,,,30\n"
            "F,2027,,,英文校閲（業者委託）,,,30\n"
            "F,2027,,,論文投稿料,,,30\n"
            "F,2027,,,クラウド計算資源利用料,,,40\n"
            "F,2027,,,研究会開催費（会場費）,,,120\n",
            encoding="utf-8")
        ...
        codes = [c for _, c, _ in check_pdf_text(
            "本研究は文科省の事業で……既存の模型[12]（150）……令和９(2027)年度……")]
        expect("表記: 略称", codes, ["ABBREV"])
        expect("表記: 裸の括弧数字", codes, ["BARE_NUMBER"])
        codes = [c for _, c, _ in check_pdf_text("文部科学省の事業。被引用 150。(2027) 年度。")]
        ...
        # 裏返し: 様式が記述を求めた欄が空のまま (実測「記載して下さい」)
        ...
        # 実例: 同じ事項が年度で違う金額 (旅費・OA 掲載料) を
        # 「異なる理由は必要性の欄に」と問われた
        vary = td / "vary.csv"
        vary.write_text(
            "費目区分,年度,品名・仕様,設置機関,事項,数量,単価,金額\n"
            "D,2027,,,国際会議での成果発表（欧州、7日間、1名）,,,250\n"
            "D,2028,,,国際会議での成果発表（欧州、7日間、1名）,,,400\n"
        ...
        # 実例: 旅費の行に参加登録料が混ざっていた
        mis = td / "misfit.csv"
        mis.write_text(
            "費目区分,年度,品名・仕様,設置機関,事項,数量,単価,金額\n"
            "D,2027,,,国際会議発表 1 件（登録料・渡航・滞在、7日間、1名）,,,400\n"
            "F,2027,,,参加登録料,,,50\n", encoding="utf-8")
        ...
        heavy.write_text(
            "費目区分,年度,品名・仕様,設置機関,事項,数量,単価,金額\n"
            "E,2027,,,博士研究員1名（月額30万円＋法定福利費、12か月）,,,4200\n"
            "B,2027,,,専門書・文献,,,100\n", encoding="utf-8")
        ...
        badpay.write_text(
            "費目区分,年度,品名・仕様,設置機関,事項,数量,単価,金額\n"
            "E,2027,,,博士研究員1名（月額30万円＋法定福利費、12か月）,,,2100\n"
            "D,2027,,,国際会議での成果発表（欧州、7日間、1名）,,,3000\n", encoding="utf-8")
        ...
        md.write_text(
            ...
            "|---|---|---|---|---|---|---|---|\n"
            "| 人件費・謝金 | 2027 |  |  | 博士研究員1名（月額30万円＋法定福利費、12か月） |  |  | 4200 |\n"
