...
def render_dir_readme(title: str, rel: str, files, note: str, lib_files=None):
    ...
    if lib_files is not None:
        lines += ["", "## lib/ — sourceable helper 群", ""]
        for name, desc in lib_files:
            lines.append(f"- **[lib/{name}](lib/{name})** — {desc}")
    return "\n".join(lines).rstrip() + "\n"
...
def build_outputs(root: Path):
    ...
    outputs[root / "scripts" / "README.md"] = render_dir_readme(
        ...
        "リポ全体の構造 tree = [CLAUDE.md](../CLAUDE.md)。", lib_files=lib)
