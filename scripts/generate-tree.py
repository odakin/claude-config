...
生成先 (5 箇所、 いずれも marker で機械管理 — 手編集禁止):
  1. CLAUDE.md          … <!-- AUTO-TREE:<name> BEGIN --> 〜 <!-- AUTO-TREE:<name> END -->
                          (name = conventions / hooks / scripts、 構造 tree の code fence 内)。
                          ⚠️ 2026-09-01 auto-load 税 縮退: conventions 行の説明は when (= trigger、
                          summary の詳細 digest は conventions/README.md 側が持つ)、 hooks/scripts は
                          per-file 行を出さず件数 + README pointer のみ (全列挙は 4./5. へ移設)。
                          理由 = CLAUDE.md は毎 session + headless routine が払う税で、 routing に
                          効くのは trigger 列 (memory-file-slimming.md の生成側適用)。
...
def collect_dir_files(root: Path, rel: str, errors: list, allow_dirs=(), tracked=None):
    ...
    for p in sorted(d.iterdir()):
        if p.name.startswith(".") or p.name == "__pycache__" or p.name.endswith(".pyc"):
            continue
        if p.name == "README.md":
            continue  # 本 script の生成物 (hooks/README.md, scripts/README.md) は源にしない
        if p.is_dir():
            if tracked is None:
...
def render_tree_conventions(entries):
    ...
    for name, meta in entries:
        if meta is None:
            ...
            items.append((name, meta["when"]))
    lines = ["├── conventions/          # ドメイン固有規約 (各行の説明 = doc-meta の when 〔いつ読むか〕。 詳細 summary + カテゴリ index = conventions/README.md)"]
...
def render_tree_hooks(files):
    return [
        ...
        f"({len(files)} file。 setup.sh が ~/.claude/hooks/ に symlink。 "
        "全列挙 + 説明 = hooks/README.md 〔生成物〕、 説明の源 = 各 file header 1 行目)",
    ]
...
def render_dir_readme(title: str, rel: str, files, note: str, subdirs=()):
    """hooks/README.md / scripts/README.md — dir 全列挙 + 説明の生成物。

    subdirs = [(dir 名, 節 title, [(file 名, 説明)])] — 直下の file の後に 1 節ずつ並べる。"""
    lines = [
        "<!-- AUTO-GENERATED by scripts/generate-tree.py — 手編集禁止。",
        f"     源 = {rel}/ 各 file header の説明 1 行目。",
    ...
    for sub, sub_title, sub_files in subdirs:
        lines += ["", f"## {sub}/ — {sub_title}", ""]
        for name, desc in sub_files:
...
def render_enum(entries):
    ...
    return (
        ...
        "`--write` を回せば全生成物へ同時反映、 drift は CI / pre-commit の `--check` が検出。 "
...
def build_outputs(root: Path):
    ...
    outputs[root / "hooks" / "README.md"] = render_dir_readme(
        "Claude Code hooks", "hooks", hooks,
        "setup.sh が `~/.claude/hooks/` に symlink する hook 群 (`*.sh` / `*.py` のみ install 対象、 "
        "本 README は対象外)。 リポ全体の構造 tree = [CLAUDE.md](../CLAUDE.md)。")
    outputs[root / "scripts" / "README.md"] = render_dir_readme(
        "運用 script 群", "scripts", scripts,
        "リポ全体の構造 tree = [CLAUDE.md](../CLAUDE.md)。", subdirs=subdirs)
...
def selftest() -> int:
    ...
    try:
        ...
        # SCRIPTS_SUBDIRS に在る subdir は lib/ 以外も自動で並ぶ (= 一覧を 2 つ持たない)
        (tmp / "scripts" / "formcase").mkdir()
        (tmp / "scripts" / "formcase" / "m.py").write_text(
            '#!/usr/bin/env python3\n"""m.py — package module の説明"""\n', encoding="utf-8")
        ...
        check("alpha.md" in claude and "# alpha を使うとき" in claude, "tree に conventions when (trigger)")
        check("alpha の規約" not in claude, "tree に summary は出ない (auto-load 税 縮退)")
        ...
        scripts_readme = (tmp / "scripts" / "README.md").read_text(encoding="utf-8")
        check("script のテスト説明" in scripts_readme and "lib helper 説明" in scripts_readme,
              "scripts/README.md に scripts + lib desc")
        check("package module の説明" in scripts_readme and "[formcase/m.py](formcase/m.py)" in scripts_readme,
              "scripts/README.md に lib/ 以外の subdir も並ぶ (SCRIPTS_SUBDIRS 1 本で決まる)")
        ...
        # 3) drift 検出
        claude = (tmp / "CLAUDE.md").read_text(encoding="utf-8")
        (tmp / "CLAUDE.md").write_text(claude.replace("# alpha を使うとき", "# 手で書き換えた"), encoding="utf-8")
        ...
        check("mjs header 説明" in (tmp / "scripts" / "README.md").read_text(encoding="utf-8"),
              ".mjs の // 説明が scripts/README.md に載る")
