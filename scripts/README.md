...
- **[gpt_measurements.py](gpt_measurements.py)** — GPT / POVM 測定の間主観性 (intersubjectivity)・sharpness・極値性を定義から機械検査する library (solver 無し: SLSQP + explicit dual certificate、多面体 GPT は LP exact、--selftest 内蔵、conventions/physics-verification-cycle.md#definition-level-judge)
