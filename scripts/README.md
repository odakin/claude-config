...
- **[latexdiff-strip-dup-labels.py](latexdiff-strip-dup-labels.py)** — Drop `\label` from the deleted side of a latexdiff output, so a changed (whole markup) or moved block keeps its label once and amsmath's "Multiple \label's" error disappears (2026-09-13; the snapshot engine applies it automatically).
