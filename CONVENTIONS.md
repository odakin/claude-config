...
9. **原稿の主張と agent の権限規約は著者のもの**: 原稿の表題・概要・序論・結論・数式と、 agent の編集権限を定める規則は、 著者の項目ごとの裁定 (著者の発言の verbatim) なしに agent が変えない。 一般的な修正依頼は裁定ではない。 正本 = [conventions/manuscript-claim-ownership.md#rule](conventions/manuscript-claim-ownership.md#rule)、 機械 gate = `scripts/manuscript-claim-guard.py` (Claude / Codex / git の 3 面)
