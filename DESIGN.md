...
**判断 3 = 書換え道具は既定値で安全側に倒し、 解除は明示 flag にする**。 [`scripts/apply-text-pairs.py`](scripts/apply-text-pairs.py): TARGET は必須引数 (既定値なし)、 挿入型 pair の再実行は拒否、 old の端が token の途中なら拒否 (`--allow-prefix` で解除)。 却下案: 冪等性と target の取り違えを patch 作者の注意に任せる (2026-09-13 に 1 回破れて engine の CLI が約 1 分落ちた = [`batch-text-edits.md#insertion-pair-rerun`](conventions/batch-text-edits.md#insertion-pair-rerun))。 [`scripts/commit-from-origin-worktree.py`](scripts/commit-from-origin-worktree.py): 丸ごとの `--copy` は DST の live HEAD 版と origin 版が違えば拒否 (`--allow-overwrite` で解除)、 rebase が衝突したら自動解決せず push もせず worktree を残す (生成物を upstream 側で取り直すかは内容の判断)。 live で push する経路自体は廃止しない (相手の未 push commit が無ければ十分) が、 その確認を注意に頼る点が near-miss の原因だったので、 相手の WIP がありうる時の既定を worktree 側に置いた ([`multi-session-coordination.md#foreign-wip-scratch-worktree`](conventions/multi-session-coordination.md#foreign-wip-scratch-worktree))。
